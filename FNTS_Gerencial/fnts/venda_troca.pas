...
begin
  ...
  if not FRMMODULO.qrproduto.Locate('CODIGO', ECODIGO.TEXT, [loCaseInsensitive]) then begin
...
///////////////////////////////////////////////////////////////////
  ...
  if not FRMMODULO.qrproduto.Locate('CODIGO', ECODIGO2.TEXT, [loCaseInsensitive]) then begin
...
begin
  if Key = #13 then begin
    if ecodigo.Text <> '' then begin
      if frmmodulo.qrproduto.Locate('codbarra', ecodigo.text, [loCaseInsensitive]) then begin
        ...
        if frmmodulo.qrproduto.Locate('codigo', ecodigo.text, [loCaseInsensitive]) then begin
          runitario.Value := frmmodulo.qrproduto.fieldbyname('precovenda').asfloat;
          rqtde.value := 1;
          rtotal.value := frmmodulo.qrproduto.fieldbyname('precovenda').asfloat;
...
begin
  if Key = vk_down then begin
    if pproduto.Visible then begin
      WWDBGRID1.SETFOCUS;
...
begin
  if Key = #13 then begin
    if EPRODUTO.TEXT <> '' then begin
      ...
      if frmmodulo.qrproduto.Locate('produto', eproduto.text, [loPartialKey]) then begin
        ecodigo.text := frmmodulo.qrproduto.fieldbyname('codigo').asstring;
        runitario.Value := frmmodulo.qrproduto.fieldbyname('precovenda').asfloat;
        rqtde.value := 1;
        rtotal.value := frmmodulo.qrproduto.fieldbyname('precovenda').asfloat;
...
begin
  if Key = #13 then begin
    if ecodigo2.Text <> '' then begin
      ...
      if frmmodulo.qrproduto.Locate('codbarra', ecodigo2.text, [loCaseInsensitive]) then begin
        ...
        if not ESTOQUE_NEGATIVO then begin
          if (qrestoque.fieldbyname('estoque_atual').asfloat - qrvendido.fieldbyname('quant').asfloat) < 0.01 then begin
        ...
        if frmmodulo.qrproduto.Locate('codigo', ecodigo2.text, [loCaseInsensitive]) then begin
...
begin
  if Key = vk_down then begin
    if pproduto2.Visible then begin
      WWDBGRID2.SETFOCUS;
...
begin
  if Key = #13 then begin
    if eproduto2.TEXT <> '' then begin
      ...
      if frmmodulo.qrproduto.Locate('produto', eproduto2.text, [loPartialKey]) then begin
