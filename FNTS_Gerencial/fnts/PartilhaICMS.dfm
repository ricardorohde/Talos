object frmPartilhaICMS: TfrmPartilhaICMS
  Left = 0
  Top = 0
  BorderStyle = bsNone
  Caption = 'frmPartilhaICMS'
  ClientHeight = 461
  ClientWidth = 814
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 814
    Height = 44
    Align = alTop
    BevelOuter = bvNone
    Color = 3355443
    ParentBackground = False
    TabOrder = 0
    ExplicitWidth = 677
    object Label26: TLabel
      Left = 55
      Top = 10
      Width = 160
      Height = 23
      Alignment = taCenter
      Caption = 'Partilha do ICMS'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -19
      Font.Name = 'Verdana'
      Font.Style = []
      ParentFont = False
      Transparent = True
    end
    object AdvMetroButton1: TAdvMetroButton
      Left = 0
      Top = 0
      Width = 49
      Height = 44
      Align = alLeft
      Appearance.PictureColor = clWhite
      Caption = ''
      Picture.Data = {
        
        65B5119A54FB20C0004E7B10B44195D3E70000000049454E44AE426082}
      TabOrder = 0
      Version = '1.1.2.0'
      OnClick = AdvMetroButton1Click
      ExplicitLeft = 4
      ExplicitTop = -7
      ExplicitHeight = 45
    end
  end
  object Panel3: TPanel
    Left = 0
    Top = 44
    Width = 814
    Height = 417
    Align = alClient
    BevelOuter = bvNone
    Color = 10943995
    ParentBackground = False
    TabOrder = 1
    ExplicitTop = 56
    ExplicitWidth = 677
    ExplicitHeight = 41
    object DBGrid1: TDBGrid
      Left = 0
      Top = 0
      Width = 814
      Height = 417
      Align = alClient
      DataSource = dsPartilha
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          FieldName = 'UF'
          ReadOnly = True
          Width = 64
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DESCR_UF'
          ReadOnly = True
          Title.Caption = 'Descri'#231#227'o'
          Width = 216
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'IBGE'
          ReadOnly = True
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ALIQ'
          Title.Caption = 'Aliq.'
          Width = 59
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ALIQREDUZBASE'
          Title.Caption = 'AlIiq. Reduz. Base'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ALIQIMPORTADO'
          Title.Caption = 'Aliq. Importado'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'IE_SUBTRIB'
          Title.Caption = 'IE Sub. Trib.'
          Width = 76
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'MVAST'
          Title.Caption = 'MVA ST'
          Width = 52
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ALIQST'
          Title.Caption = 'Aliq ST'
          Width = 55
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'FCP'
          Title.Caption = 'F.C.P.'
          Width = 51
          Visible = True
        end>
    end
  end
  object dsPartilha: TDataSource
    DataSet = qrPartilha
    Left = 225
    Top = 146
  end
  object qrPartilha: TZQuery
    Connection = frmmodulo.econexao2
    AfterPost = qrPartilhaAfterPost
    BeforeDelete = qrPartilhaBeforeDelete
    OnNewRecord = qrPartilhaNewRecord
    SQL.Strings = (
      'select * from ICM_PARTILHA')
    Params = <>
    Left = 168
    Top = 142
    object qrPartilhaCODIGO: TIntegerField
      FieldName = 'CODIGO'
    end
    object qrPartilhaUF: TWideStringField
      FieldName = 'UF'
      Size = 2
    end
    object qrPartilhaDESCR_UF: TWideStringField
      FieldName = 'DESCR_UF'
      Size = 50
    end
    object qrPartilhaIBGE: TWideStringField
      FieldName = 'IBGE'
      Size = 2
    end
    object qrPartilhaALIQ: TFloatField
      FieldName = 'ALIQ'
    end
    object qrPartilhaALIQREDUZBASE: TFloatField
      FieldName = 'ALIQREDUZBASE'
    end
    object qrPartilhaALIQIMPORTADO: TFloatField
      FieldName = 'ALIQIMPORTADO'
    end
    object qrPartilhaIE_SUBTRIB: TWideStringField
      FieldName = 'IE_SUBTRIB'
      Size = 18
    end
    object qrPartilhaMVAST: TFloatField
      FieldName = 'MVAST'
    end
    object qrPartilhaALIQST: TFloatField
      FieldName = 'ALIQST'
    end
    object qrPartilhaFCP: TFloatField
      FieldName = 'FCP'
    end
  end
end
