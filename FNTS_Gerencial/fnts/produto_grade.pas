...
uses
  modulo, produto_entrada, principal;
...
begin
  ...
  if QRGRADE_ENTRADA.RecordCount = 0 then begin
    ...
    RTOTAL.Value := frmproduto_entrada.qrentrada_item.fieldbyname('qtde').asfloat;
    while not qrgrade_entrada.eof do begin
...
begin
  ...
  if (rdiferenca.value <> 0) then begin
    if rdigitado.Value <> 0 then begin
...
begin
  if frmmodulo.qrgrade_produto.Locate('codigo', qrgrade_entrada.fieldbyname('codgrade').asstring, [loCaseInsensitive]) then begin
