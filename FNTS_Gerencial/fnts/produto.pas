...
uses
  ...
  RzPanel, AdvReflectionImage, ACBrBarCode, VCLTee.TeEngine, VCLTee.Series,
  VCLTee.TeeProcs, VCLTee.Chart, IdBaseComponent, IdComponent,
  IdCustomTransparentProxy, IdSocks, UCBase, W7Labels, dxGDIPlusClasses,
  ...
  cxDropDownEdit, cxLookupEdit, cxDBLookupEdit, cxDBExtLookupComboBox, cxMemo,
  cxImageComboBox, System.ImageList, Vcl.ImgList, cxButtonEdit, cxCurrencyEdit,
...
type
  Tfrmproduto = class(TForm)
    ...
    procedure btnCodigoEANClick(Sender: TObject);
  private
    { Private declarations }
    UsaCadastroSimples: Boolean;
  public
    { Public declarations }
  end;
...
uses
  modulo, principal, loc_grupo, loc_fornecedor, loc_subgrupo, loc_marca, receita,
  loc_receita, webcam, fornecedor, marca, grupo, aliquota, cfop,
  produto_exportar, produto_importar, produto_total, lista_produto2,
  xloc_produto, produto_atualizapreco, xloc_cst, inventario, baixa_estoque,
  servico, loc_aliquota, Produto_consultaserial, produto_movimentar, unNFe2,
  produto_serial_ficha, fornecedor_codigo, Ncm, CEST, xloc_csosn,
  ConversaoUnidade, xloc_cfop, produto_importaEAN;
...
begin
  if frmprincipal.autentica('Excluir PRODUTO', 4) then begin
    ...
    qrcodbarra.OPEN;
    while qrcodbarra.RECORDCOUNT <> 0 do begin
      qrcodbarra.DELETE;
    end;
...
begin
  tedit(Sender).Color := clwindow;
  if (qrproduto.State = dsinsert) or (qrproduto.State = dsedit) then begin
    if ealiquota.Text <> '' then begin
      if not frmprincipal.Locregistro(FRMMODULO.qrALIQUOTA, ealiquota.Text, 'codigo') then
...
begin
  try
    ...
    if FRMMODULO.qrgrade_subgrupo.RECORDCOUNT > 0 then begin
      ...
      Showmessage('A família a qual este produto pertence não possue grade! Favor verificar!');
    end;
  except
  end;
end;
...
{    sSql := 'insert into EXTRA_PRODUTOS (COD_PROD, ID_EXTRA, VALOR) values (:COD_PROD, :ID_EXTRA, :VALOR)';
    qrTmp.Close;
    qrTmp.SQL.Clear;
    qrTmp.SQL.Text := sSql;
    qrTmp.ParamByName('COD_PROD').AsString := qrprodutoCODIGO.AsString;
    qrTmp.ParamByName('ID_EXTRA').AsInteger := qrExtrasID.AsInteger;
    qrTmp.ParamByName('VALOR').AsFloat := qrExtrasVALOR.AsFloat;
    qrTmp.ExecSQL;}
  end
  else begin
    if qrExtraProduto.Locate('ID_EXTRA', qrExtrasID.AsInteger, [loCaseInsensitive]) then begin
    ...
    {sSql := 'delete from EXTRA_PRODUTOS where id = (select ex.id from extra_produtos ex where ex.id_extra = ' + qrExtrasID.AsString + ' and ex.cod_prod = ' +  QuotedStr(qrprodutoCODIGO.AsString)+')';
    qrTmp.Close;
    qrTmp.SQL.Clear;
    qrTmp.SQL.Text := sSql;
    qrTmp.ExecSQL;}
...
begin
  if frmprincipal.autentica('Atualizar Produtos', 4) then begin
...
begin
  if Key = #13 then begin
    PageView1.ActivePageindex := 0;
    PageView2.ActivePageindex := 0;
    if gestoque.visible then begin
...
begin
  ...
  if qrcodbarra.State = dsinsert then begin
    ...
    qrbarra.OPEN;
    if qrbarra.RECORDCOUNT > 0 then begin
...
begin
  ...
  if resultado_pesquisa1 <> '' then begin
    query.Close;
    query.sql.clear;
    query.SQL.Add('select * from c000025 where codigo = ' + QuotedStr(resultado_pesquisa1));
    query.Open;
    if query.RecordCount > 0 then begin
    ...
    if (qrproduto.State = dsinsert) or (qrproduto.State = dsedit) then begin
      ...
      if esubgrupo.Text <> '000000' then
        if not frmprincipal.Locregistro(FRMMODULO.qrSUBGRUPO, esubgrupo.Text, 'codigo') then begin
          application.messagebox('Família não cadastrada!', 'Atenção', mb_ok + mb_iconerror);
          esubgrupo.SetFocus;
          exit;
        end
        else
...
begin
  try
    if PageView1.ActivePageindex = 8 then begin
      ...
      qrvenda_mes.sql.add('select sum(qtde) sum_0, ' + '(select sum(qtde) from c000032 where movimento in (2,9) and data >= :data11 and data < :data10 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + '''), ' + '(select sum(qtde) from c000032 where movimento in (2,9) and data >= :data10 and data < :data09 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + '''), ' +
        '(select sum(qtde) from c000032 where movimento in (2,9) and data >= :data09 and data < :data08 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + '''), ' + '(select sum(qtde) from c000032 where movimento in (2,9) and data >= :data08 and data < :data07 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + '''), ' + '(select sum(qtde) from c000032 where movimento in (2,9) and data >= :data07 and data < :data06 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString
        + '''), ' + '(select sum(qtde) from c000032 where movimento in (2,9) and data >= :data06 and data < :data05 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + '''), ' + '(select sum(qtde) from c000032 where movimento in (2,9) and data >= :data05 and data < :data04 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + '''), ' + '(select sum(qtde) from c000032 where movimento in (2,9) and data >= :data04 and data < :data03 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString
        + '''), ' + '(select sum(qtde) from c000032 where movimento in (2,9) and data >= :data03 and data < :data02 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + '''), ' + '(select sum(qtde) from c000032 where movimento in (2,9) and data >= :data02 and data < :data01 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + '''), ' + '(select sum(qtde) from c000032 where movimento in (2,9) and data >= :data01 and data < :data00 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + ''') ' + 'from c000032 where movimento in (2,9) and data >= :data12 and data < :data11 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + '''');
...
{      Series1.clear;
      ...
      qrvenda_mes.close;
      qrvenda_mes.sql.clear;
      qrvenda_mes.sql.add('select sum(qtde) sum_0, ' + '(select sum(qtde) from c000032 where MOVIMENTO = 1 and data >= :data11 and data < :data10 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + '''), ' + '(select sum(qtde) from c000032 where MOVIMENTO = 1 and data >= :data10 and data < :data09 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + '''), ' + '(select sum(qtde) from c000032 where MOVIMENTO = 1 and data >= :data09 and data < :data08 and codproduto = ''' + qrproduto.FieldByName
        ('codigo').AsString + '''), ' + '(select sum(qtde) from c000032 where MOVIMENTO = 1 and data >= :data08 and data < :data07 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + '''), ' + '(select sum(qtde) from c000032 where MOVIMENTO = 1 and data >= :data07 and data < :data06 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + '''), ' + '(select sum(qtde) from c000032 where MOVIMENTO = 1 and data >= :data06 and data < :data05 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString
        + '''), ' + '(select sum(qtde) from c000032 where MOVIMENTO = 1 and data >= :data05 and data < :data04 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + '''), ' + '(select sum(qtde) from c000032 where MOVIMENTO = 1 and data >= :data04 and data < :data03 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + '''), ' + '(select sum(qtde) from c000032 where MOVIMENTO = 1 and data >= :data03 and data < :data02 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString +
        '''), ' + '(select sum(qtde) from c000032 where MOVIMENTO = 1 and data >= :data02 and data < :data01 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + '''), ' + '(select sum(qtde) from c000032 where MOVIMENTO = 1 and data >= :data01 and data < :data00 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + ''') ' + 'from c000032 where MOVIMENTO = 1 and data >= :data12 and data < :data11 and codproduto = ''' + qrproduto.FieldByName('codigo').AsString + '''');
...
begin
  if Key = #13 then begin
    if eCEST.Text = '' then
      Exit;
    if (qrproduto.State = dsinsert) or (qrproduto.State = dsedit) then begin
      qrproduto.FieldByName('CEST').AsString := frmprincipal.zerarcodigo(eCEST.Text, 6);
      if eCEST.Text <> '000000' then
        if not frmprincipal.Locregistro(FRMMODULO.qrCest, eCEST.Text, 'CEST') then begin
          application.messagebox('CEST não encontrado!', 'Atenção', mb_ok + mb_iconerror);
          eCEST.SetFocus;
          exit;
        end
        else
...
begin
  tedit(Sender).Color := clwindow;
  if DBDateEdit2.date < DBDateEdit1.date then begin
...
begin
  if Exp_Abre_PRODUTO('PRODUTO.txt') then begin
    Exp_Inclui_PRODUTO(qrproduto.FieldByName('codigo').AsString, qrproduto.FieldByName('produto').AsString, qrproduto.FieldByName('codbarra').AsString, '', '', '', qrproduto.FieldByName('unidade').AsString, qrproduto.FieldByName('precovenda').asfloat, '', '', 0, 0, 0, '', '', '', qrproduto.FieldByName('cst').AsString, 0, 1, qrproduto.FieldByName('aliquota').asfloat, 0, '', 0, 0);

    if Exp_Fecha_PRODUTO then begin
...
begin
  ...
  if Exp_Abre_PRODUTO('PRODUTO.txt') then begin
    while not query.Eof do begin
      ...
      Exp_Inclui_PRODUTO(query.FieldByName('codigo').AsString, query.FieldByName('produto').AsString, query.FieldByName('codbarra').AsString, '', '', '', query.FieldByName('unidade').AsString, query.FieldByName('precovenda').asfloat, '', '', 0, 0, 0, '', '', '', query.FieldByName('cst').AsString, 0, 1, query.FieldByName('aliquota').asfloat, 0, '', 0, 0);
      query.next;
    end;
    if Exp_Fecha_PRODUTO then begin
      Showmessage('Arquivo de PRODUTO criado com sucesso!');
...
procedure Tfrmproduto.btnCodigoEANClick(Sender: TObject);
var
frmproduto_importaEAN: Tfrmproduto_importaEAN;
begin

frmproduto_importaEAN := Tfrmproduto_importaEAN.Create(self);

frmproduto_importaEAN.Show();

//frmproduto_importaEAN.Close;
//
//
//
//
//Self.Show;
//Self.COMBOCODBARRA.SetFocus;
...
begin
  ...
  else if not (Key in ['0'..'9', #08]) then
    Key := #0;
