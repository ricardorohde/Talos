      ...
      begin
        try
          ...
          textox := PWideChar('Não foi possível se conectar o banco de dados: ' + #13 +
            qrpdv.fieldbyname('hostname').asstring + ':C:\TALOS\bd\base.fdb' + #13 +
...
begin
  ...
  AssignFile(TXT, 'C:\TALOS\server\CG' + IntToStr(ANO) + IntToStr(MES) + IntToStr(DIA) + '_' + IntToStr(HORA) + IntToStr(MINUTO) + IntToStr(SEGUNDO) + '.001');
  ...
  bar.Percent := 0;
  origem := 'C:\TALOS\server\CG' + IntToStr(ANO) + IntToStr(MES) + IntToStr(DIA) + '_' + IntToStr(HORA) + IntToStr(MINUTO) + IntToStr(SEGUNDO);
