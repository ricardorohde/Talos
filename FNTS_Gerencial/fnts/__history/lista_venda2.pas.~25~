...
//    FRMMODULO.qrrelatorio.fieldbyname('LINHA5').asstring := FRMMODULO.qrrelatorio.fieldbyname('LINHA5').asstring + '  SITUAÇÃO: ' + COMBO_SITUACAO.Text;
  ...
  if combo_relatorio.Text = 'RESUMO DE VENDAS POR VENDEDOR' then begin
    QRVENDA2.CLOSE;
    QRVENDA2.SQL.CLEAR;
    QRVENDA2.SQL.Add('select nfce.CODVENDEDOR, SUM(nfce.TOTAL) TOTAL_GERAL from c000048 nfce where ' + periodo + cliente + VENDEDOR + caixa + FORMA + SITUACAO + ' GROUP BY CODVENDEDOR');
    QRVENDA2.SQL.Add('UNION');
    QRVENDA2.SQL.Add('select nfe.CODVENDEDOR, SUM(nfe.TOTAL_NOTA) TOTAL_GERAL from c000061 nfe where ' + periodo + cliente + VENDEDOR + caixa + FORMA + ' AND NFE_SITUACAO = 6  GROUP BY CODVENDEDOR');
