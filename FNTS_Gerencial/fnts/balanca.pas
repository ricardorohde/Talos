...
uses
  ...
  ExtCtrls, Collection, TFlatPanelUnit, ComCtrls, Menus,
  AdvGlowButton, RzPrgres, Vcl.Imaging.jpeg, AdvMetroButton, AdvSmoothPanel,
...
begin
  ...
  if application.messagebox('Deseja diferenciar Peso de Unidade?', 'Aviso', mb_yesNo + mb_iconquestion) = idYes then
    pz := true
  else
    pz := false;
  if (combobalanca.Text = 'FILIZOLA') or (combobalanca.text = 'URANO') then begin
    ...
    rewrite(txt_receita);
    rewrite(txt_nutri);
    while not frmModulo.qrproduto.eof do begin
      if (frmModulo.qrproduto.fieldbyname('codbarra').asstring <> '') and (length(frmModulo.qrproduto.fieldbyname('codbarra').asstring) < 6) then begin
        ...
        if FRMMODULO.QRPRODUTO.FIELDBYNAME('PRECO_PROMOCAO').ASFLOAT <> 0 then begin
          if (DATE >= FRMMODULO.QRPRODUTO.FIELDBYNAME('DATA_PROMOCAO').ASDATETIME) and (DATE <= FRMMODULO.QRPRODUTO.FIELDBYNAME('FIM_PROMOCAO').ASDATETIME) then begin
...
{  if combobalanca.text = 'URANO' then begin
    //URANO_PRODUTO.open;
//    while URANO_PRODUTO.recordcount <> 0 do URANO_PRODUTO.delete;
  ...
  if combobalanca.text = 'TOLEDO' then begin
    memo1.Text := '';
    assignfile(txt_PRODUTO, FRMMODULO.QRCONFIG.FIELDBYNAME('BALANCA_CAMINHO').ASSTRING + '\TXITENS.TXT');
    rewrite(txt_PRODUTO);
    while not frmModulo.qrproduto.eof do begin
      texto := frmModulo.qrproduto.fieldbyname('CODIGO').asstring;
      texto1 := frmModulo.qrproduto.fieldbyname('produto').asstring;
      if FRMMODULO.QRPRODUTO.FIELDBYNAME('PRECO_PROMOCAO').ASFLOAT <> 0 then begin
        if (DATE >= FRMMODULO.QRPRODUTO.FIELDBYNAME('DATA_PROMOCAO').ASDATETIME) and (DATE <= FRMMODULO.QRPRODUTO.FIELDBYNAME('FIM_PROMOCAO').ASDATETIME) then begin
          texto2 := formatfloat('0.00', frmModulo.qrproduto.fieldbyname('preco_promocao').asfloat);
      ...
      TEXTO4 := '0';
      if pz then begin
        if UpperCase(frmModulo.qrproduto.fieldbyname('UNIDADE').asstring) = 'KG' then
          texto4 := '0';
        if UpperCase(frmModulo.qrproduto.fieldbyname('UNIDADE').asstring) = 'UN' then
          texto4 := '1';
      end;
      while length(texto) < 6 do
        texto := '0' + texto;
      while length(texto1) < 50 do
        texto1 := texto1 + ' ';
      while length(texto1) > 50 do
        delete(texto1, 51, 1);
      delete(texto2, pos(',', texto2), 1);
      while length(texto2) < 6 do
        texto2 := '0' + texto2;
      while length(texto3) < 3 do
        texto3 := '0' + texto3;
      if frmmodulo.qrproduto.fieldbyname('codreceita').asstring <> '' then
        texto5 := frmModulo.QRPRODUTO.fieldbyname('receita').asstring
      else
        texto5 := '';
      while length(texto5) < 250 do
        texto5 := texto5 + ' ';
      while length(texto5) > 250 do
        delete(texto5, 251, 1);
  ...
  if application.messagebox('Deseja iniciar o software da balança?', 'Aviso', mb_yesno + mb_iconquestion) = idyes then begin
    frmmodulo.qrconfig.open;
