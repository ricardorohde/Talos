...
type
  Tfrmlista_venda2 = class(TForm)
    ...
    qrVenda_Detalhado: TZQuery;
    qrvenda_produto10DATA: TDateField;
    qrvenda_produto10CUPOM_MODELO: TWideStringField;
    qrvenda_produto10CODCLIENTE: TWideStringField;
    qrvenda_produto10CODVENDEDOR: TWideStringField;
    qrvenda_produto10vendedor: TStringField;
    qrvenda_produto10cliente: TStringField;
...
uses
  ...
  xloc_fornecedor, xloc_marca, produto, fornecedor;
...
begin
  ...
  begin
    ...
    try
      ExecSQL;
      SQL.Clear;
      SQL.Add('ALTER TABLE c000061 ADD MEIO_CARTAODEB decimal(18,4) default 0, ADD MEIO_CREDIARIO decimal(18,4) default 0, ADD MEIO_CONVENIO decimal(18,4) default 0, ADD MEIO_FINANCEIRA decimal(18,4) default 0');
      ExecSQL;
    except
...
//    FRMMODULO.qrrelatorio.fieldbyname('LINHA5').asstring := FRMMODULO.qrrelatorio.fieldbyname('LINHA5').asstring + '  SITUAÇÃO: ' + COMBO_SITUACAO.Text;
  ...
  if combo_relatorio.Text = 'RESUMO DE VENDAS DE PRODUTOS' then begin
    ...
    begin
      ...
      if bTodos_Produtos.Checked = false then
        qrvenda_produto10.SQL.Add('select data, codproduto, avg(unitario) media_venda, sum(qtde) total_qtde,  ( avg(unitario) * sum(qtde)) - sum(desconto) total_geral, cupom_modelo, codcliente, codvendedor from c000032 where  movimento in (2,9) and movimento_estoque < 0 and ' + periodo + SITUACAO + cliente + VENDEDOR + PRODUTO + ' group by data, codproduto, codcliente, codvendedor, cupom_modelo order by data, codproduto')
      else
        qrvenda_produto10.SQL.Add('select data, codproduto, avg(unitario) media_venda, sum(qtde) total_qtde,  ( avg(unitario) * sum(qtde)) - sum(desconto) total_geral, cupom_modelo, codcliente, codvendedor from c000032 where  movimento in (2,9,99) and ' + periodo + SITUACAO + cliente + VENDEDOR + PRODUTO + ' group by data, codproduto,, codcliente, codvendedor, cupom_modelo order by data, codproduto');
  ...
  if combo_relatorio.Text = 'VENDAS POR VENDEDOR / LUCRO' then begin
      qrvenda_produto10.CLOSE;
      qrvenda_produto10.SQL.CLEAR;
  ...
  if combo_relatorio.Text = 'RESUMO DE VENDAS POR VENDEDOR' then begin
    ...
    begin
      Close;
      SQL.Clear;
      SQL.Add('SELECT nfce.CODVENDEDOR, SUM(nfce.TOTAL) TOTAL_GERAL');
      if combo_forma.Text = 'TODAS' then
      begin
        SQL.Add(', SUM(nfce.MEIO_DINHEIRO) AS TotDIN, SUM(nfce.MEIO_CHEQUEAV) AS TotCQAV, SUM(nfce.MEIO_CHEQUEAP) AS TotCQAP, ');
        SQL.Add('SUM(nfce.MEIO_CARTAOCRED) AS TotCC, SUM(nfce.MEIO_CARTAODEB) AS TotCD, SUM(nfce.MEIO_CREDIARIO) AS TotCDO, SUM(nfce.MEIO_CONVENIO) AS TotCV, SUM(nfce.MEIO_FINANCEIRA) AS TotFIN ');
      end;
      SQL.Add('FROM c000048 nfce WHERE ' + periodo + cliente + VENDEDOR + caixa + FORMA + SITUACAO + ' GROUP BY CODVENDEDOR ');
      SQL.Add('UNION');
      SQL.Add('SELECT nfe.CODVENDEDOR, SUM(nfe.TOTAL_NOTA) TOTAL_GERAL');
      if combo_forma.Text = 'TODAS' then
      begin
        SQL.Add(', SUM(nfe.MEIO_DINHEIRO) AS TotDIN, SUM(nfe.MEIO_CHEQUEAV) AS TotCQAV, SUM(nfe.MEIO_CHEQUEAP) AS TotCQAP, ');
        SQL.Add('SUM(nfe.MEIO_CARTAOCRED) AS TotCC, SUM(nfe.MEIO_CARTAODEB) AS TotCD, SUM(nfe.MEIO_CREDIARIO) AS TotCDO, SUM(nfe.MEIO_CONVENIO) AS TotCV, SUM(nfe.MEIO_FINANCEIRA) AS TotFIN');
      end;
      SQL.Add('FROM c000061 nfe WHERE ' + periodo + cliente + VENDEDOR + caixa + FORMA + ' AND NFE_SITUACAO = 6 GROUP BY CODVENDEDOR');
      Params.ParamByName('datai').asdatetime := DateEdit1.Date;
      Params.ParamByName('dataf').asdatetime := DateEdit2.Date;
      Open;
    end;
