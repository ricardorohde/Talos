...
uses
  ...
  ListaNfeBaixar, dxSkinsCore, dxSkinBlack, dxSkinBlue, dxSkinBlueprint,
...
type
  ...
  public
    ...
    function CadastrarProduto(CodProdutoXML, CodBarras, Produto, CodFornecedor, Unidade, NCM, csosn, cst: string; IPI, qtdEmbalagem: double): Boolean;
...
uses
  ...
  UFuncoes, status, chavenfe_importar, xloc_produto, ACBrDFeSSL, blcksock,
  System.TypInfo;
...
begin
  ...
  if qrpagar.RecordCount > 0 then begin
...
//    if Length(qritemCODBARRA.AsString) = 0 then begin
//      MessageBeep(64);
//      application.messagebox('Atenção!' + #13#10 + 'Existe Produto que precisa ser localizado para referenciamento ao cadastro, VERIFIQUE.', 'Importação de XML', mb_ok + MB_ICONWARNING);
//      abort;
//    end;
    if frmprincipal.isFloat(qritemPRECOVENDA.AsString) <= 0 then begin
      MessageBeep(64);
  ...
  if qritem.RecordCount > 0 then begin
    ...
    while not qritem.eof do begin
      ...
      if qritemCODPRODUTO.AsString <> '' then begin
        query.Close;
        query.sql.Clear;
        ...
        else begin { : CADASTRA O PRODUTO E ATUALIZA O CODPRODUTO NO QRITEM }
          CadastrarProduto(qritemCODLANCAMENTO.AsString, qritemCODBARRA.AsString, qritemPRODUTO.AsString, qritemCODFORNECEDOR.AsString, qritemUN_COMPRA.AsString, qritemCLASSIFICACAO_FISCAL.AsString, qritemCSOSN.AsString, qritemCST.AsString, qritemIPI_ALIQUOTA.ASFLOAT, qritemFRACAO.ASFLOAT);

          if Length(qritemCODPRODUTO.AsString) = 0 then begin
            qritem.edit;
        ...
        if qritemFATOR.AsFloat > 0 then begin
          if qritemTIPOFATOR.AsString = 'M' then begin
            qrproduto_movQTDE.ASFLOAT := qritemQTDE.ASFLOAT * qritemFATOR.ASFLOAT;
            qrproduto_movUNITARIO.ASFLOAT := qritemUNITARIO.ASFLOAT / qritemFATOR.ASFLOAT;
          end
          else begin
            qrproduto_movQTDE.ASFLOAT := qritemQTDE.ASFLOAT / qritemFATOR.ASFLOAT;
            qrproduto_movUNITARIO.ASFLOAT := qritemUNITARIO.ASFLOAT * qritemFATOR.ASFLOAT;
          ...
          qrproduto_movUNITARIO.ASFLOAT := qritemUNITARIO.ASFLOAT / qritemFRACAO.ASFLOAT;
        end;
        ...
        lstatus.caption := '[ ' + qritem.fieldbyname('codproduto').AsString + ' ] ' + qritem.fieldbyname('codbarra').AsString + ' | ' + qritem.fieldbyname('produto').AsString;
        application.ProcessMessages;

      end // Se existir produto sem ligação ao cadastro
      else begin
        CadastrarProduto(qritemCODLANCAMENTO.AsString, qritemCODBARRA.AsString, qritemPRODUTO.AsString, qritemCODFORNECEDOR.AsString, qritemUN_FRACAO.AsString, qritemCLASSIFICACAO_FISCAL.AsString, qritemCSOSN.AsString, qritemCST.AsString, qritemIPI_ALIQUOTA.ASFLOAT, qritemFRACAO.ASFLOAT);
  ...
  begin
    ...
    frmmodulo.qrproduto.sql.add('SELECT * FROM C000025 WHERE CODIGO = ''' + qritemCODPRODUTO.AsString + '''');
    ...
    qrfornecedor_codigo.sql.add('and CODPRODUTO = ' + QuotedStr(qritemCODPRODUTO.AsString));
    ...
    frmmodulo.qrprodutoPRECOCUSTO.Value := RoundTo(qritemPRECOCUSTO.ASFLOAT, -2);

    if qritemFATOR.AsFloat > 0 then begin
      if qritemTIPOFATOR.AsString = 'M' then begin
        frmmodulo.qrprodutoESTOQUE.Value := frmmodulo.qrprodutoESTOQUE.Value + (qritemQTDE.ASFLOAT * qritemFATOR.ASFLOAT);
      end
      else begin
        frmmodulo.qrprodutoESTOQUE.Value := frmmodulo.qrprodutoESTOQUE.Value + (qritemQTDE.ASFLOAT / qritemFATOR.ASFLOAT);
      end;
      ...
      frmmodulo.qrprodutoESTOQUE.Value := frmmodulo.qrprodutoESTOQUE.Value + (qritemQTDE.ASFLOAT * qritemFRACAO.ASFLOAT);
    end;

    frmmodulo.qrprodutoPRECOVENDA.Value := RoundTo(qritemPRECOVENDA.ASFLOAT, -2);
    frmmodulo.qrprodutoPMARGEM1.Value := RoundTo(qritemPMARGEM.ASFLOAT, -2);
    ...
    if Length(frmmodulo.qrprodutoCLASSIFICACAO_FISCAL.AsString) = 0 then
      frmmodulo.qrprodutoCLASSIFICACAO_FISCAL.Value := qritemCLASSIFICACAO_FISCAL.AsString;

    frmmodulo.qrproduto.Post;

    qryestoque.Close;
    qryestoque.sql.Clear;
    qryestoque.sql.add('SELECT * FROM C000100 WHERE CODPRODUTO = ' + QuotedStr(qritemCODPRODUTO.AsString));
    qryestoque.sql.add('AND CODFILIAL = ' + QuotedStr(frmmodulo.qrFilialCODIGO.AsString));
    qryestoque.open;
    qryestoque.edit;
    ...
    frmmodulo.qrpreco.sql.add('select * from c000026 where codproduto = ''' + qritemCODPRODUTO.AsString + '''');
    ...
    frmmodulo.qrpreco.fieldbyname('codproduto').AsString := qritemCODPRODUTO.AsString;
    frmmodulo.qrpreco.fieldbyname('DATA_ALTERACAO').Value := date;
    frmmodulo.qrprecoVCUSTO_COMPRA.Value := RoundTo(qritemPRECOCUSTO.ASFLOAT, -2);
    frmmodulo.qrprecoPRECOVAREJO1.Value := RoundTo(qritemPRECOVENDA.ASFLOAT, -2);
    frmmodulo.qrprecoCP_PIPI.Value := qritemIPI_ALIQUOTA.ASFLOAT;
    frmmodulo.qrprecoCP_VIPI.Value := qritemIPI_VALOR.ASFLOAT;

    if qritemFATOR.AsFloat > 0 then begin
      if qritemTIPOFATOR.AsString = 'M' then begin
        frmmodulo.qrprecoCP_VFRETE.Value := ((qritemFRETE.ASFLOAT / qritemQTDE.ASFLOAT) / qritemFATOR.ASFLOAT);
        frmmodulo.qrprecoCP_VSEGURO.Value := ((qritemSEGURO.ASFLOAT / qritemQTDE.ASFLOAT) / qritemFATOR.ASFLOAT);
        frmmodulo.qrprecoCP_VOUTROS.Value := ((qritemOUTRAS.ASFLOAT / qritemQTDE.ASFLOAT) / qritemFATOR.ASFLOAT);
      end
      else begin
        frmmodulo.qrprecoCP_VFRETE.Value := ((qritemFRETE.ASFLOAT / qritemQTDE.ASFLOAT) * qritemFATOR.ASFLOAT);
        frmmodulo.qrprecoCP_VSEGURO.Value := ((qritemSEGURO.ASFLOAT / qritemQTDE.ASFLOAT) * qritemFATOR.ASFLOAT);
      ...
      frmmodulo.qrprecoCP_VFRETE.Value := ((qritemFRETE.ASFLOAT / qritemQTDE.ASFLOAT) / qritemFRACAO.ASFLOAT);
      frmmodulo.qrprecoCP_VSEGURO.Value := ((qritemSEGURO.ASFLOAT / qritemQTDE.ASFLOAT) / qritemFRACAO.ASFLOAT);
      frmmodulo.qrprecoCP_VOUTROS.Value := ((qritemOUTRAS.ASFLOAT / qritemQTDE.ASFLOAT) / qritemFRACAO.ASFLOAT);
    end;
    ...
    { varejo }
    frmmodulo.qrprecoPRECOVAREJO1.Value := RoundTo(qritemPRECOVENDA.ASFLOAT, -2);
    frmmodulo.qrprecoPMARGEM1.Value := RoundTo(qritemPMARGEM.ASFLOAT, -2);
    ...
    if qritemFATOR.AsFloat > 0 then begin
      if qritemTIPOFATOR.AsString = 'M' then begin
        frmmodulo.qrprecoPRECOATACADO1.Value := RoundTo(qritemPRECOVENDA.ASFLOAT / qritemFATOR.ASFLOAT, -2);
      end
      else begin
        frmmodulo.qrprecoPRECOATACADO1.Value := RoundTo(qritemPRECOVENDA.ASFLOAT * qritemFATOR.ASFLOAT, -2);
      end;
      ...
      frmmodulo.qrprecoPRECOATACADO1.Value := RoundTo(qritemPRECOVENDA.ASFLOAT * qritemFRACAO.ASFLOAT, -2);
    end;
...
begin
  ...
  {chave := RetornaChaveDigitada;
  if chave <> '' then begin
    ACBrNFe1. DistribuicaoDFe(UFtoCUF(frmmodulo.qrFilialUF.asstring),SN(frmmodulo.qrFilialCNPJ.asstring),'','',Chave);
    ...
    ACBrNFe1.DistribuicaoDFePorChaveNFe(UFtoCUF(frmmodulo.qrFilialUF.asstring),
                          SN(frmmodulo.qrFilialCNPJ.asstring),
                          chave);
    ...
    if ACBrNFe1.WebServices.DistribuicaoDFe.retDistDFeInt.cStat = 138 then begin
      if FileExists(ACBrNFe1.Configuracoes.Arquivos.PathSalvar + chave + '-nfe.xml') then
        CarregaDadosXML(ACBrNFe1.Configuracoes.Arquivos.PathSalvar + chave + '-nfe.xml')
      else
        Application.MessageBox(PWideChar('Não localizado o xml baixado do Sefaz:'+#13+ACBrNFe1.Configuracoes.Arquivos.PathSalvar + chave + '-nfe.xml'),'Atenção!',MB_ICONINFORMATION);
    end else begin
      Application.MessageBox(PWideChar('Não foi possivel baixar o XML da Sefaz Motivo: '+sMotivo),'Atenção!',MB_ICONINFORMATION);
    end;
  end;}
...
function Tfrmimporta_nfe.CadastrarProduto(CodProdutoXML, CodBarras, Produto, CodFornecedor, Unidade, NCM, csosn, cst: string; IPI, qtdEmbalagem: double): Boolean;
var
  icms: Double;
begin
  query.Close;
  ...
  if (qritemCFOP_CAD.AsString = '5102') or (qritemCFOP_CAD.AsString = '6102') then begin
    QryCadastraProdutoIND_CFOP_VENDA_DENTRO.AsString := '5102';
    QryCadastraProdutoIND_CFOP_VENDA_FORA.AsString := '6102';
  end
  else if Pos(qritemCFOP_CAD.AsString, '5405, 5403, 5408, 5401, 6405, 6403, 6408, 6401') > 0 then begin
...
var
  x, i: integer;
  vid, vnat, vnota, vserie, vcfop, vmodelo, vemissao, vsaida, vuf, vcodmunicipio, vdv, vemitente, vdestinatario, vemitente_cnpj, vdestinatario_cnpj, vcod_item, vcod_fornecedor, vcod_produto, vcidade, vcep, vcomplemento, vtelefone1, vtelefone2, vfax, vcontato1, vcontato2, vcelular1, vcelular2, vemail, vhomepage, vcnpj, vie, vbanco, vagencia, vconta, vobs1, vobs2, vobs3, vnumero, vim, vcod_municipio_ibge, vibge, vnomeprod, vxBairro, vxLgr, vfantasia, vmunemitente, vmundestinatario, vdata, idLote, TipoFator, lMsg, scfpotmp: string;
  xvnomeprod, vcadprodnovo, vtipo, vassistencia_tecnica: integer;
  va, Fator, vfracao: double;
begin
  try
    continua := true;
    ...
    else begin { : SE O FORNECEDOR NÃO ESTIVER CADASTRADO - CADASTRA }
      CadastraFornecedor;
    ...
    for x := 0 to eitems.AsInteger - 1 do begin
      ...
      if qrfornecedor_codigo.RecordCount > 0 then begin
      ...
      else begin { : Se nao tiver associado ao fornecedor }
        ...
        if (frmprincipal.somenteNumero(ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.cEAN) <> '') then begin
        ...
        else begin // Se nao tiver cod Barras e nao tiver referenciado
          vcod_produto := '';
          vfracao := 1;
          ...
          query.sql.Clear;
          query.sql.add('select * from c000025');
          query.sql.add('where codbarra = ''' + ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.cProd + '''');
          query.open;
          ...
          if query.RecordCount > 0 then begin
            ...
            // inicio
//            vcodBarra_produto := '978100' + frmprincipal.zerarcodigo(IntToStr(strtoint(vcod_item)), 6);
//            vcodBarra_produto := vcodBarra_produto + frmprincipal.CalculaDigEAN13(frmprincipal.somenteNumero(vcodBarra_produto));
      ...
      if vcod_produto <> '' then begin
        ...
        qrConversao.SQL.Add('where uni.codproduto = ' + QuotedStr(Trim(vcod_produto)) + ' and upper(uni.un_origem) = ' + UpperCase(QuotedStr(Trim(ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.uTrib))) + ' and upper(uni.un_destino) = ' + UpperCase(QuotedStr(Trim(query.FieldByName('UNIDADE').AsString))));
        qrConversao.Open;
        if not qrConversao.IsEmpty then begin
          qritemFATOR.AsFloat := qrConversaoFATOR.AsFloat;
          qritemTIPOFATOR.AsString := qrConversaoTIPO.AsString;
        end;
        with frmmodulo do begin
          qrpreco.Close;
          qrpreco.SQL.Clear;
          qrpreco.SQL.Add('select * from c000026 where CODPRODUTO = ' + QuotedStr('vcod_produto'));
          qrpreco.Open;
          if not qrpreco.IsEmpty then begin
            if qrpreco.FieldByName('PRECO_AUTOMATICO').AsInteger = 1 then begin
              qrpreco.Edit;
              qrpreco.FieldByName('VALOR_ULTIMA_COMPRA').asfloat := ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.vProd - ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.vDesc;
              ...
              qrpreco.FieldByName('PRECO_VENDA').asfloat := qrpreco.FieldByName('CUSTO_COMPRA').asfloat + qrpreco.FieldByName('LUCRO').asfloat + qrpreco.FieldByName('TOTAL_CUSTO').asfloat;
              qrpreco.Post;
              qrpreco.ApplyUpdates;
              vPrecoVenda := qrpreco.FieldByName('PRECO_VENDA').asfloat;
            end;
          end;
        end;
      ...
      if Length(CodBarrasValido(CodigoProdutoNovo)) > 0 then
        ...
        qritemCODBARRA.AsString := CodBarrasValido(ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.cProd);

      if (Length(ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.cEAN) > 0) and (Length(vcodBarra_produto) = 0) then
        qritemCODBARRA.AsString := ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.cEAN;
      ...
      if Length(NomeProduto) > 0 then
        ...
        qritemPRODUTO.AsString := ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.xProd;
      ...
      if (scfpotmp = '51') or (scfpotmp = '61') then begin
        ...
        qritemCSOSN.AsString := '102';
      end
      else if (scfpotmp = '54') or (scfpotmp = '64') then begin
        qritemCST.AsString := '060';
        qritemCSOSN.AsString := '500';
      end
      else begin
        qritemCST.AsString := '041';
        qritemCSOSN.AsString := '400';
      end;

      // CST
{      if ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Imposto.ICMS.cst = cstVazio then
        qritemCST.AsString :=  ''
      else
        qritemCST.AsString :=  '0'+CSTICMSToStr(ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Imposto.ICMS.cst);

      //CSOSN
      if  ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Imposto.ICMS.CSOSN = csosnVazio then
        qritemCSOSN.AsString := ''
      else
        qritemCSOSN.AsString := CSOSNIcmsToStr(ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Imposto.ICMS.CSOSN);
 }

      // CFOP
      qritemCFOP_CAD.AsString := ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.CFOP;
      ...
      qritemTOTAL.ASFLOAT := ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.vProd - ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.vDesc;
      ...
      { : PREÇOS }
      qritemPRECOCUSTO.ASFLOAT := RoundTo((qritemTOTAL.ASFLOAT + qritemFRETE.ASFLOAT + qritemSEGURO.ASFLOAT + qritemOUTRAS.ASFLOAT + qritemIPI_VALOR.ASFLOAT + qritemSUB_VALOR.ASFLOAT) / (qritemQTDE.ASFLOAT) / qritemFRACAO.ASFLOAT, -2);
      ...
      if vPrecoVenda > 0 then
        ...
        qritemPRECOVENDA.ASFLOAT := RoundTo(qritemPRECOCUSTO.ASFLOAT / qritemFRACAO.ASFLOAT, -2);

      qritemPMARGEM.Value := RoundTo((((qritemPRECOVENDA.ASFLOAT - qritemPRECOCUSTO.ASFLOAT) / qritemPRECOCUSTO.ASFLOAT)) * 100, -2);
      { : PREÇOS }
      ...
      lstatus.caption := '[ ' + frmprincipal.zerarcodigo(IntToStr(ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.nItem), 3) + ' ] ' + ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.cEAN + ' | ' + ACBrNFe1.NotasFiscais.Items[0].NFe.Det.Items[x].Prod.xProd;
      application.ProcessMessages;
...
begin
  if not (qritem.Active) or (qritem.IsEmpty) then begin
  ...
  if edAtual.Checked then begin
    qritem.Edit;
    qritemPRECOVENDA.AsFloat := qritemPRECOCUSTO.AsFloat + ((qritemPRECOCUSTO.AsFloat * edLucro.Value) / 100);
    qritem.Post;
    qritem.ApplyUpdates;
  end
  else begin
...
procedure Tfrmimporta_nfe.blocprodutoClick(Sender: TObject);
begin
  if not (qritem.Active) or (qritem.IsEmpty) then begin
    Application.MessageBox('Não existe itens para linkar a um produto.', 'Atenção!', MB_ICONINFORMATION);
    Exit;
  end;
  if trim(qritemCODPRODUTO.AsString) <> '' then begin
    if Application.MessageBox('O Item selecionado ja esta linkado a um produto, deseja linkar a outro?.', 'Atenção!', MB_ICONQUESTION + MB_YESNO) <> mrYes then
      Exit;
  end;
  resultado_pesquisa1 := '';
  ...
  if resultado_pesquisa1 = '' then
    ...
    qritem.Edit;
    qritemCODPRODUTO.AsString := resultado_pesquisa1;
    with frmmodulo do begin
      qrpreco.Close;
      qrpreco.SQL.Clear;
      qrpreco.SQL.Add('select * from c000026 where CODPRODUTO = ' + QuotedStr(resultado_pesquisa1));
      qrpreco.Open;
      if not qrpreco.IsEmpty then begin
        if qrpreco.FieldByName('PRECO_AUTOMATICO').AsInteger = 1 then begin
          qrpreco.Edit;
          qrpreco.FieldByName('VALOR_ULTIMA_COMPRA').asfloat := qritemPRECOCUSTO.AsFloat;
          qrpreco.FieldByName('CUSTO_COMPRA').asfloat := (qrpreco.FieldByName('VALOR_ULTIMA_COMPRA').asfloat + qrpreco.FieldByName('FRETE').asfloat + qrpreco.FieldByName('SEGURO').asfloat + qrpreco.FieldByName('OUTRAS_DESPESAS').asfloat + qrpreco.FieldByName('IPI').asfloat) - (qrpreco.FieldByName('ICMS_ENTRADA').asfloat + qrpreco.FieldByName('DESCONTO').asfloat);
          qrpreco.FieldByName('NOVO_PRECO_CUSTO').asfloat := qrpreco.FieldByName('CUSTO_COMPRA').asfloat;
          qrpreco.FieldByName('TOTAL_CUSTO_P').asfloat := qrpreco.FieldByName('ICMS_SAIDA_P').asfloat + qrpreco.FieldByName('PIS_P').asfloat + qrpreco.FieldByName('COFINS_P').asfloat + qrpreco.FieldByName('CONTRIBUICAO_SOCIAL_P').asfloat + qrpreco.FieldByName('CUSTO_OPERACIONAL_P').asfloat + qrpreco.FieldByName('COMISSAO_P').asfloat;
          qrpreco.FieldByName('TOTAL_CUSTO').asfloat := qrpreco.FieldByName('ICMS_SAIDA').asfloat + qrpreco.FieldByName('PIS').asfloat + qrpreco.FieldByName('COFINS').asfloat + qrpreco.FieldByName('CONTRIBUICAO_SOCIAL').asfloat + qrpreco.FieldByName('CUSTO_OPERACIONAL').asfloat + qrpreco.FieldByName('COMISSAO').asfloat;
          ...
          qrpreco.Post;
          qrpreco.ApplyUpdates;
          qritemPRECOVENDA.AsFloat := qrpreco.FieldByName('PRECO_VENDA').asfloat;
        end;
      end;
    end;
    qritem.Post;
...
begin
  ...
  if frmmodulo.QRCONFIG.fieldbyname('CADASTRO_PRODUTO').AsString = 'NORMAL' then begin
   ...
   //   frmproduto_LIGTH.showmodal;
      ...
      if frmmodulo.QRCONFIG.fieldbyname('CADASTRO_PRODUTO').AsString = 'FARMA' then begin
...
begin
  ...
  fximprime.LoadFromFile('\TALOS\server\REL\F000197.fr3');
...
procedure Tfrmimporta_nfe.eprodutoExit(Sender: TObject);
begin
  if qritem.State <> dsEdit then
    qritem.Edit;
  qritemCODPRODUTO.asstring := frmPrincipal.zerarcodigo(eproduto.Text, 6);
  if trim(eproduto.Text) <> '' then begin
    with frmmodulo do begin
      qrpreco.Close;
      qrpreco.SQL.Clear;
      qrpreco.SQL.Add('select * from c000026 where CODPRODUTO = ' + QuotedStr(frmPrincipal.zerarcodigo(eproduto.Text, 6)));
      qrpreco.Open;
      if not qrpreco.IsEmpty then begin
        if qrpreco.FieldByName('PRECO_AUTOMATICO').AsInteger = 1 then begin
          qrpreco.Edit;
...
begin
  ...
  for i := 1 to ecount.IntValue do begin
    ...
    qrpagarCODIGO.AsString := frmprincipal.codifica('000046');
    ;
    qrpagarDOCUMENTO.AsString := enota.Text + '/' + frmprincipal.zerarcodigo(IntToStr(i), 2);
    qrpagarESPECIE.AsString := 'DUPLICATA';
    qrpagarHISTORICO.AsString := 'Duplicata a Pagar';
