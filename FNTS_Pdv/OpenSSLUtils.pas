...
type
  ...
  public
    ...
    function IsTrusted(CACertificate: array of TX509Certificate)
      : boolean; overload;
  ...
  public
    ...
    procedure LoadPrivateKey(KeyFilename: TFileName;
      KeyPassword: ansistring); overload;
    procedure LoadCertificate(CertificateFilename: TFileName);
    property Password: ansistring write fPassword;
    property PrivateKeyFile: TFileName read fPrivateKeyFile
      write fPrivateKeyFile;
    property CertificateFile: TFileName read fCertificateFile
      write fCertificateFile;
...
begin
  ...
  if FindFirst(TmpDir + '\*', faReadOnly and faHidden and faSysFile and
    faArchive, TmpFile) = 0 then
...
procedure TKeyPairGenerator.KeyFileNames(PrivateKeyName, PublicKeyName
  : TFileName);
begin
...
//  BIO_write_filename(PublicKeyOut, pansichar(fPublicKeyFile));
  ...
  PEM_write_bio_RSAPrivateKey(PrivateKeyOut, rsa, enc, nil, 0, nil,
    PChar(fPassword));
...
begin
  ...
  if (asn1_time.asn1_type <> V_ASN1_UTCTIME) and
    (asn1_time.asn1_type <> V_ASN1_GENERALIZEDTIME) then
  ...
  begin
    ...
    if (buffer[10] >= '0') and (buffer[10] <= '9') and (buffer[11] >= '0') and
      (buffer[11] <= '9') then
  ...
  begin
    ...
    if (buffer[12] >= '0') and (buffer[12] <= '9') and (buffer[13] >= '0') and
      (buffer[13] <= '9') then
...
//  BIO_read_filename(certfile, pansichar(FileName));
  ...
  BIO_free(certfile);
  if fCertificate = nil then
    raise EOpenSSL.Create('Unable to read certificate from file ' +
      FileName + '.');
end;

function TX509Certificate.VerifyCalback(ok: integer;
  ctx: pX509_STORE_CTX): integer;
var
...
function TX509Certificate.IsTrusted(CACertificate
...
begin
  ...
  certfile := BIO_new(BIO_s_file());
  BIO_ctrl(certfile, BIO_C_SET_FILENAME, BIO_CLOSE or BIO_FP_READ,
    PChar(ansistring(CertificateFilename)));
  fCertificate := PEM_read_bio_X509_AUX(certfile, c, nil, nil);
  if fCertificate = nil then
