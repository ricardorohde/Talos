...
type
  TfrmProduto_consulta = class(TForm)
    ...
    procedure ed_barraKeyUp(Sender: TObject; var Key: Char; Shift: TShiftState);
...
// -------------------------------------------------------------------------- //
procedure TfrmProduto_consulta.ed_barraKeyUp(Sender: TObject; var Key: Char; Shift: TShiftState);
begin
 //if key = #13 then
  //begin
    //Key := #0;
...
//        iPesquisa_produto := 0;
  ...
  //begin
    ...
    begin
      ...
      LocalizarProduto(ANSIUPPERCASE('*'));
    end;

    ed_barra.SetFocus();
    ed_barra.SelStart:= Length(ed_barra.Text);
...
procedure TfrmProduto_consulta.pnFundoClick(Sender: TObject);
begin
