...
uses
  ...
  Dialogs, Menus, StdCtrls, Buttons, ExtCtrls, Collection, Mask, TFlatPanelUnit,  AdvGlowButton, AdvShapeButton, ComCtrls,
  wwdbdatetimepicker, JvExMask, JvToolEdit, JvBaseEdits;
...
uses contasreceber, modulo, principal,Math, contasreceber_banco,
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          frmmodulo.qrcaixa_mov.fieldbyname('codcaixa').asstring    := Zerar(IntToStr(iNumCaixa),6);
          frmmodulo.qrcaixa_mov.fieldbyname('codoperador').asstring := Zerar(IntToStr(icodigo_Usuario),6);
        ...
        begin
          frmmodulo.qrcaixa_mov.OPEN;
          frmmodulo.qrcaixa_mov.insert;
          frmmodulo.qrcaixa_mov.fieldbyname('codcaixa').asstring    := IntToStr(iNumCaixa);
          frmmodulo.qrcaixa_mov.FieldByName('codigo').asstring      := frmprincipal.codifica('000044',8);
          frmmodulo.qrcaixa_mov.fieldbyname('codoperador').asstring := Zerar(IntToStr(icodigo_Usuario),6);
          frmmodulo.qrcaixa_mov.fieldbyname('data').asstring        := frmmodulo.qrcaixa_operador.FIELDBYNAME('DATA').ASSTRING;
       ...
       vqtde_parcelas := vqtde_parcelas-1;
        ...
        begin
          ...
          begin
            ...
            frmcontasreceber.qrcontasreceber.fieldbyname('codvendedor').asstring     := Zerar(IntToStr(icodigo_Usuario),6);
            frmcontasreceber.qrcontasreceber.fieldbyname('codcaixa').asstring        := IntToStr(iNumCaixa);
...
begin
 {
       ...
       begin
        repeat
          ...
          sMsg := cECF_Recebimento(iECF_Modelo,sTotalizador_recebimento,rtotal.value, 'Dinheiro');
          ...
          begin
            ...
            frmmodulo.spNao_fiscal.ParamByName('codvendedor').AsInteger := icodigo_Usuario;
     ...
     begin // Nao Fiscal
           ...
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'<e>   RECIBO DE PAGAMENTO</e>');
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'------------------------------------------------');
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'Cliente: <b>'+copy(frmmodulo.qrcliente.fieldbyname('nome').asstring,1,38)+'</b>');
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'CPF: <b>'+frmmodulo.qrcliente.fieldbyname('cpf').asstring+'</b>');
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'------------------------------------------------');
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'Recebemos do cliente acima informado, a importância'+
                                              'de <b>('+extenso1.extenso(frmcontasreceber_pgto.rtotal.value)+')</b>'+
                                              'referente ao recebimento da(s) prestção(ões) abaixo relacionada(s):');
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'------------------------------------------------');
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'Vencimento  Cupom      Documento      Valor - R$');
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'------------------------------------------------');
          ...
          while not frmcontasreceber.qrcr.Eof do begin
           ...
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'------------------------------------------------');
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'                                TOTAL:'+  Texto_justifica(FORMATFLOAT('###,###,##0.00',frmcontasreceber_pgto.rliquido.value  ),10,' ',taDireita));
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'                             DESCONTO:'+  Texto_justifica(FORMATFLOAT('###,###,##0.00',frmcontasreceber_pgto.rdesconto2.value),10,' ',taDireita));
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'                            ACRESCIMO:'+  Texto_justifica(FORMATFLOAT('###,###,##0.00',frmcontasreceber_pgto.rjuros.value    ),10,' ',taDireita));
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'                            PAGAMENTO:'+  Texto_justifica(FORMATFLOAT('###,###,##0.00',frmcontasreceber_pgto.rsoma.value     ),10,' ',taDireita));
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'                             RESTANTE:'+  Texto_justifica(FORMATFLOAT('###,###,##0.00',frmcontasreceber_pgto.rdiferenca.value),10,' ',taDireita));
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'------------------------------------------------');
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'Para maior clareza firmamos o presente.         ');
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'');
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal, FormatDateTime('dddd", "d" de "mmmm" de "yyyy',date)+'.');
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'');
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'');
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'');
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'------------------------------------------------');
           sMsg := Imp_Usa_Gerencial(sPortaNaoFiscal,'          Assinatura do Funcionário');
