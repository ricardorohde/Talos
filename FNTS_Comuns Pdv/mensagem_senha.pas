...
begin
  if autenticado then begin
    ...
    if frmprincipal.msg('ERRO', 'Acesso não permitido!' + #13 + 'Tentar outra vez?', true, true, false, '') = 'SIM' then begin
      COMBOBOX1.SETFOCUS;
    end else begin
...
begin
  if combobox1.Text <> '' then begin
    if not frmmodulo.qrUsuario.LOCATE('nome', COMBOBOX1.TEXT, [LOPARTIALKEY]) then begin
      if frmprincipal.msg('ERRO', 'Usuário não cadastrado!' + #13 + 'Tentar outra vez?', true, true, false, '') = 'SIM' then begin
        combobox1.text := '';
        combobox1.setfocus;
      end
      else begin
    ...
    try
      ...
      if edit2.text = SENHA then begin
        ...
        if autenticado then begin
          ...
          if APPLICATION.MESSAGEBOX('Sem permissão para conceder desconto!' + #13 + 'Deseja tentar outra vez?', 'Atenção', mb_yesno + mb_iconerror) = idYes then begin
            edit2.text := '';
            edit2.setfocus;
          end else begin
            RESULTADO_MENSAGEM := 'NAO';
