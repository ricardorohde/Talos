...
uses
  ...
  extenso1, notafiscal_importacupom, sintegra_gerar, OpNatureza, Vendedor;
...
var
  ...
  if check_prestacao.Checked then begin
    ...
    if query2.RecordCount > 0 then begin
      application.messagebox('Esta nota fiscal possui prestações que já teve pagamentos efetuados! Não será possível recriar as prestações!', 'Aviso', mb_ok + mb_iconwarning);
...
{          if qrnotafiscal_item.fieldbyname('BASE_CALCULO').AsFloat > 0 then
    ...
    frmnotafiscal_menu.qrnota.fieldbyname('
