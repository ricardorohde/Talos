...
type
  ...
  Tfrmnotafiscal_menu = class(TForm)
    ...
    bt_nfe_inutilizar: TAdvGlowButton;
    ...
    qryTmp: TZQuery;
    qrduplicatas: TZQuery;
    qryaux: TZQuery;
    qrnotaRET_VALOR_PIS: TFloatField;
    qrnotaRET_VALOR_COFINS: TFloatField;
    qrnotaRET_VALOR_CSLL: TFloatField;
    qrnotaRET_VALOR_BC_IRRF: TFloatField;
    qrnotaRET_VALOR_IRRF: TFloatField;
    qrnotaRET_VALOR_BC_PREV_SOC: TFloatField;
    qrnotaRET_VALOR_PREV_SOC: TFloatField;
    qrNFE_ClienteCPAIS: TWideStringField;
    qrNFE_ClienteDPAIS: TWideStringField;
    qrnotaPRESENCA_COMPRADOR: TIntegerField;
    qrnotaVFCP: TFloatField;
    qrnotaVFCPST: TFloatField;
    qrnotaVFCPSTRET: TFloatField;
    qrnotaVIPIDEVOL: TFloatField;
    pn_nfe_log: TPanel;
    Panel7: TPanel;
    ...
    Panel10: TPanel;
    Button1: TButton;
    qrNFE_ClienteTIPO_IE: TWideStringField;
    qrNFE_ClienteCONSUMIDOR_FINAL: TWideStringField;
    ACBrNFe1: TACBrNFe;
...
uses
  modulo, principal, compra, cfop, cst, modelo_fiscal, xloc_cliente, compra_item,
  lista_notaentrada, lista_notacompra, notafiscal, compra_menu, lista_notasaida,
  notafiscal_cancelar, unNFe2, status, nfe_legenda, email, math, cliente,
  produto, fornecedor, empresa, OpNatureza, orcamento, notas_venda,
  ResultadoWebService, JustificativaInutilizacaoNFe, Texto, UFuncoes;
...
//         AtualizaSSLLibsCombo;
     ...
     // cbUF.ItemIndex        := 'BA'///cbUF.Items.IndexOf(Ini.ReadString( 'WebService','UF','SP')) ;
      ...
      ACBrNFe1.SSL.SSLType := TSSLType( Ini.ReadInteger('WebService','SSLType' , 0) );
      ...
      if ACBrNFe1.DANFE <> nil then
       begin
         ACBrNFe1.DANFE.TipoDANFE  := StrToTpImp(OK,IntToStr(Ini.ReadInteger( 'DANFE','Tipo'       ,0)+1));
  ...
  with frmmodulo do begin
    ...
    if qrDFeConfigNFE_DANFE.AsInteger = 0 then
      ACBrNFe1.DANFE := ACBrNFeDANFEFR1
    else
      ACBrNFe1.DANFE := ACBrNFeDANFCeFortes1;
    ACBrNFe1.SSL.DescarregarCertificado;
    ACBrNFe1.Configuracoes.Geral.SSLLib := TSSLLib(qrDFeConfigTSSLLIB.AsInteger);
    ACBrNFe1.Configuracoes.Geral.SSLCryptLib := TSSLCryptLib(qrDFeConfigTSSLCRYPTLIB.AsInteger);
    ACBrNFe1.Configuracoes.Geral.SSLHttpLib := TSSLHttpLib(qrDFeConfigTSSLHTTPLIB.AsInteger);
    ACBrNFe1.Configuracoes.Geral.SSLXmlSignLib := TSSLXmlSignLib(qrDFeConfigTSSLXMLSIGNLIB.AsInteger);
    ACBrNFe1.Configuracoes.Geral.AtualizarXMLCancelado := qrDFeConfigATUALIZAR_XML.AsString = 'S';
    ACBrNFe1.Configuracoes.Geral.ExibirErroSchema := qrDFeConfigEXIBIR_ERRO_SCHEMA.AsString = 'S';
    ACBrNFe1.Configuracoes.Geral.RetirarAcentos := qrDFeConfigRETIRAR_ACENTOS.AsString = 'S';
    ACBrNFe1.Configuracoes.Geral.FormatoAlerta := qrDFeConfigFORMATO_ALERTA.AsString;
    ACBrNFe1.Configuracoes.Geral.FormaEmissao := TpcnTipoEmissao(qrDFeConfigNFE_FORMATO_EMISSAO.AsInteger);
    ACBrNFe1.Configuracoes.Geral.ModeloDF := moNFe;
    ACBrNFe1.Configuracoes.Geral.VersaoDF := TpcnVersaoDF(qrDFeConfigNFE_VERSAO.AsInteger);
    ACBrNFe1.Configuracoes.Geral.Salvar := qrDFeConfigNFE_SALVAR_ENVIO_RESPOSTA.AsString = 'S';
    if ACBrNFe1.Configuracoes.Geral.VersaoDF = ve400 then
      VersaoNFE := v400
    else
      VersaoNFE := v310;
    ACBrNFe1.Configuracoes.WebServices.UF := qrDFeConfigWEBSERVICE_UF.AsString;
    ACBrNFe1.Configuracoes.WebServices.Ambiente := StrToTpAmb(ok, IntToStr(qrDFeConfigAMBIENTE.AsInteger + 1));
    ACBrNFe1.Configuracoes.WebServices.Visualizar := qrDFeConfigVISUALIZAR_MENSAGEM.AsString = 'S';
    ACBrNFe1.Configuracoes.WebServices.Salvar :=  qrDFeConfigSALVAR_ENVELOPE_SOAP.AsString = 'S';
    ACBrNFe1.Configuracoes.WebServices.AjustaAguardaConsultaRet := qrDFeConfigAJUSTAR_AGUARDAR.AsString = 'S';
    if qrDFeConfigAGUARDAR.AsInteger > 0 then
      ACBrNFe1.Configuracoes.WebServices.AguardarConsultaRet := ifThen(qrDFeConfigAGUARDAR.AsInteger < 1000, qrDFeConfigAGUARDAR.AsInteger * 1000, qrDFeConfigAGUARDAR.AsInteger);
    if qrDFeConfigTENTATIVAS.AsInteger > 0 then
      ACBrNFe1.Configuracoes.WebServices.Tentativas := qrDFeConfigTENTATIVAS.AsInteger;

    if qrDFeConfigINTERVALO.AsInteger > 0 then
      ACBrNFe1.Configuracoes.WebServices.IntervaloTentativas := ifThen(qrDFeConfigINTERVALO.AsInteger < 1000, qrDFeConfigINTERVALO.AsInteger * 1000, qrDFeConfigINTERVALO.AsInteger);
    ACBrNFe1.Configuracoes.WebServices.TimeOut := qrDFeConfigTIMEOUT.AsInteger;
    ACBrNFe1.Configuracoes.WebServices.ProxyHost := qrDFeConfigPROXY_HOST.AsString;
    ACBrNFe1.Configuracoes.WebServices.ProxyPort := qrDFeConfigPROXY_PORTA.AsString;
    ACBrNFe1.Configuracoes.WebServices.ProxyUser := qrDFeConfigPROXY_USUARIO.AsString;
    ACBrNFe1.Configuracoes.WebServices.ProxyPass := qrDFeConfigPROXY_SENHA.AsString;
...
var
  ...
  lDifal, lBaseProduto, lBaseProduto1, lIcmDifal, lValFCP, lPartDest, lPartOrig: Currency;
  lALiqICmsInterEstadual, lALiqICmsInterno, lAliqIcmsImportado, lAliqFCP: Currency;
  lAliqAux: Currency;
  TotalPago:Double;
begin
  ...
  with ACBrNFe1.NotasFiscais.add.NFe do begin
    ...
    qryapoio.close;
    qryapoio.sql.clear;
    qryapoio.sql.add('select * from NFE_COMPLEMENTAR where NUMNOTA = :NUMNOTA');
    qryapoio.ParamByName('NUMNOTA').asstring := qrnota.fieldbyname('numero').asstring;
    qryapoio.open;

    if qryapoio.RecordCount > 0 then begin
      // Para NFe referenciada use os campos abaixo
      with Ide.NFref.add do begin
        refNFe := qryapoio.fieldbyname('chave').asstring; // NFe Eletronica
      end;
    end;

    //Contador
    if (query2.fieldbyname('CONTADOR_CNPJ').asstring <> '') then
      autXML.Add.CNPJCPF := SN(query2.fieldbyname('CONTADOR_CNPJ').asstring);
    ...
    Dest.EnderDest.cPais := StrToInt(qrNFE_Cliente.fieldbyname('cpais').asstring);
    Dest.EnderDest.xPais := qrNFE_Cliente.fieldbyname('dpais').asstring;
    ...
    if qrNFE_Cliente.fieldbyname('uf').asstring = 'EX' then begin
      Dest.EnderDest.xMun := qrNFE_Cliente.fieldbyname('dpais').asstring;
      with exporta do begin
        UFSaidaPais := query2.fieldbyname('UF').asstring;
        xLocExporta := query2.fieldbyname('CIDADE').asstring;
        xLocDespacho := query2.fieldbyname('CIDADE').asstring;
      end;
    end else begin
    ...
    if (query2.fieldbyname('UF').asstring = qrNFE_Cliente.fieldbyname('UF').asstring) then
      ...
      if qrNFE_Cliente.fieldbyname('uf').asstring = 'EX' then
        Ide.idDest := doExterior
      else
        Ide.idDest := doInterestadual;


    if qrnfe_cliente.fieldbyname('consumidor_final').AsString = 'S' then
      ide.indFinal := cfConsumidorFinal
    else
      ide.indFinal := cfNao;


    if qrnfe_cliente.fieldbyname('TIPO').Asinteger = 1 then begin
      Dest.indIEDest := inNaoContribuinte;
    end;

    if qrnfe_cliente.fieldbyname('TIPO').Asinteger = 2 then begin
      if qrnfe_cliente.fieldbyname('tipo_ie').asstring = 'CONTRIBUINTE' then begin
      ...
      end else if qrnfe_cliente.fieldbyname('tipo_ie').asstring = 'NÃO CONTRIBUINTE' then begin
        Dest.IE := qrnfe_cliente.fieldbyname('rg').asstring;
        Dest.indIEDest := inNaoContribuinte;
      end;
    end;
    ...
    qrnota_item.sql.add('select c000062.*, c000025.produto, c000025.codbarra cod_barra, c000025.unidade un, c000025.informa_codigo_barra_xml, c000025.CODIGO_ANP, anp.descricao anp_desc');
    qrnota_item.sql.add('from c000062');
    qrnota_item.sql.add('  inner join c000025 on c000062.codproduto = c000025.codigo');
    qrnota_item.sql.add('  left join anp on c000025.CODIGO_ANP = anp.codigo');
    qrnota_item.sql.add('where c000062.codnota = ''' + qrnota.fieldbyname('codigo').asstring + '''');
    qrnota_item.open;
    qrnota_item.first;
    ...
    if frmmodulo.qrconfig.fieldbyname('CADASTRO_PRODUTO').asstring = 'PECAS' then begin
      ...
      query.open;
      if query.fieldbyname('CODRECEITA').asstring = 'SIM' then begin
        BnF_VEICULO := True;
      end;
    end;
    if BnF_VEICULO then begin
      with Det.Add do begin
        ...
        Prod.xProd := Copy(query.fieldbyname('veic_especie').asstring, 1, 3) + '/' + query.fieldbyname('veic_tipo').asstring;
        infAdProd := query.fieldbyname('veic_cod_marca').asstring + '/' + qrnota_item.fieldbyname('produto').asstring + ';' +
'Ano: ' + query.fieldbyname('veic_ano_fabricacao').asstring + ' ' + 'Mod: ' + query.fieldbyname('veic_ano_modelo').asstring + ';' +
...
'Categoria: ' + query.fieldbyname('VEIC_NUMERO_MOTOR').asstring;
        ...
        Prod.vProd := qrnota_item.fieldbyname('subtotal').asfloat;
        Prod.cEANTrib := '';
        ...
        with Imposto do begin
          ...
          case iCRT of
            1:
              begin // Linhas para o simples nacional
                ICMS.CSOSN := StrToCSOSNIcms(vok, Copy(qrnota_item.fieldbyname('csosn').asstring, 1, 3));
            ...
            3:
              begin
                ICMS.cst := StrToCSTICMS(vok, Copy(qrnota_item.fieldbyname('cst').asstring, 2, 3));
                case qrnota_item.fieldbyname('origem').AsInteger of
                  0:
                    ICMS.orig := oeNacional;
                  1:
          ...
          with qrTrib do begin
            if (qrnota_item.fieldbyname('origem').AsInteger = 0) or (qrnota_item.fieldbyname('origem').AsInteger = 3) or (qrnota_item.fieldbyname('origem').AsInteger = 4) or (qrnota_item.fieldbyname('origem').AsInteger = 5) or (qrnota_item.fieldbyname('origem').AsInteger = 8) then begin
              close;
              sql.clear;
              sql.add('select ALIQNAC from IBPT_ALIQUOTAS where NCM = ''' + qrnota_item.fieldbyname('CLASSIFICACAO_FISCAL').asstring + '''');
              open;
              if qrTrib.RecordCount > 0 then begin
                vTotTrib := RoundTo((qrnota_item.fieldbyname('total').Value * qrTrib.fieldbyname('ALIQNAC').asfloat) / 100, -2);
                xTotalImposto := xTotalImposto + vTotTrib;
              end;
              TotalIBPT := RoundTo(TotalIBPT + ((Prod.vProd * qrTrib.fieldbyname('ALIQNAC').asfloat) / 100), -2);
            end
            else begin
              close;
              sql.clear;
              sql.add('select ALIQIMP from IBPT_ALIQUOTAS where NCM = :NCM');
              ParamByName('NCM').asstring := frmPrincipal.somenteNumero(qrnota_item.fieldbyname('CLASSIFICACAO_FISCAL').asstring);
              open;
              if qrTrib.RecordCount > 0 then begin
                vTotTrib := RoundTo((qrnota_item.fieldbyname('total').Value * qrTrib.fieldbyname('ALIQIMP').asfloat) / 100, -2);
                xTotalImposto := xTotalImposto + vTotTrib;
              end;
              TotalIBPT := RoundTo(TotalIBPT + ((Prod.vProd * qrTrib.fieldbyname('ALIQIMP').asfloat) / 100), -2);
            end;
          end;
      ...
      while not qrnota_item.eof do begin
        with Det.Add do begin
          ...
          Prod.nItem := qrnota_item.fieldbyname('item').asinteger;
          Prod.cProd := qrnota_item.fieldbyname('codproduto').asstring;
          if qrnota_item.fieldbyname('informa_codigo_barra_xml').asstring = 'S' then begin
            Prod.cEAN := Trim(qrnota_item.fieldbyname('codbarra').asstring);
          end else
            Prod.cEAN := 'SEM GTIN';
          // frmPrincipal.Zerarcodigo(qrnota_item.fieldbyname('codbarra').asstring, 13);
          // NCM
          if frmmodulo.qrproduto.Locate('codigo', qrnota_item.fieldbyname('codproduto').asstring, [loCaseInsensitive]) then begin
            Prod.NCM := frmPrincipal.somenteNumero(frmmodulo.qrproduto.fieldbyname('CLASSIFICACAO_FISCAL').asstring);
            Prod.xProd := frmmodulo.qrproduto.fieldbyname('produto').asstring;
            if (frmmodulo.qrproduto.fieldbyname('cest').AsString <> '') and ((trim(frmmodulo.qrproduto.fieldbyname('cest').AsString) <> '0')) then
              prod.CEST := frmmodulo.qrproduto.fieldbyname('cest').AsString;
          end;
          Prod.cfop := qrnota_item.fieldbyname('cfop').asstring;
          Prod.uCom := qrnota_item.fieldbyname('UN').asstring;
          Prod.qCom := qrnota_item.fieldbyname('qtde').asfloat;
          ...
          Prod.vFrete := RoundTo((qrnota_item.fieldbyname('subtotal').asfloat / qrnota.fieldbyname('VALOR_PRODUTOS').asfloat) * qrnota.fieldbyname('frete').AsCurrency, -2);
          Prod.vSeg := RoundTo((qrnota_item.fieldbyname('subtotal').asfloat / qrnota.fieldbyname('VALOR_PRODUTOS').asfloat) * qrnota.fieldbyname('seguro').AsCurrency, -2);
          Prod.vOutro := RoundTo((qrnota_item.fieldbyname('subtotal').asfloat / qrnota.fieldbyname('VALOR_PRODUTOS').asfloat) * qrnota.fieldbyname('outras_despesas').AsCurrency, -2);
          if AtivaB2B then begin
            Prod.xPed := qrnota_item.fieldbyname('xped').AsString;
            Prod.nItemPed := frmPrincipal.zerarcodigo(Trim(qrnota_item.fieldbyname('item').AsString),6);
            infAdProd := 'item="'+qrnota_item.fieldbyname('xitem').AsString+'" ped="'+qrnota_item.fieldbyname('xped').AsString+'" uM="'+qrnota_item.fieldbyname('UN').asstring+'"';
          end;
          if trim(qrnota_item.FieldByName('CODIGO_ANP').AsString) <> '' then begin
            Prod.comb.cProdANP := qrnota_item.FieldByName('CODIGO_ANP').AsInteger;
            Prod.comb.UFcons := Dest.EnderDest.UF;
            prod.comb.descANP := qrnota_item.FieldByName('anp_desc').AsString;
          end;

          Prod.EXTIPI := '23';
          if qrnota_item.fieldbyname('informa_codigo_barra_xml').asstring = 'S' then begin
            Prod.cEANTrib := Trim(qrnota_item.fieldbyname('codbarra').asstring);
          end else
            Prod.cEANTrib := 'SEM GTIN';
          // Prod.IndTot   := qrnota.fieldbyname('total_nota').AsVariant;
          Prod.IndTot := TpcnIndicadorTotal(0);
          if Trim(qrnota_item.fieldbyname('serial').asstring) <> '' then
            infAdProd := 'N.Serie: ' + Trim(qrnota_item.fieldbyname('serial').asstring);
          with Imposto do begin
            iCRT := StrToInt(frmmodulo.qrfilial.fieldbyname('crt').asstring);
            case iCRT of
              1:
                begin // Linhas para o simples nacional
                  ...
                  case ICMS.CSOSN of
                    ...
                    // Colocar o percentual do Crédito
                        ICMS.vCredICMSSN := (qrnota_item.fieldbyname('valor_icms').asfloat * qrnota_item.fieldbyname('ICMS').asfloat) / 100;
                    // Colocar o valor do Crédito
                      end;
                  // qrnota_item.fieldbyname('base_calculo').asfloat
                    csosn102:
                      begin
                        ICMS.modBCST := dbisMargemValorAgregado;
                        ICMS.pMVAST := qrnota_item.fieldbyname('margem_agregada').AsFloat;
                        ICMS.pRedBCST := 00.00;
                        ICMS.vBCST := qrnota_item.fieldbyname('base_sub').AsFloat;
                        ICMS.pICMSST := 00.00;
                    ...
                    csosn500:
                      begin
                        ICMS.vBCSTRet := qrnota_item.fieldbyname('base_sub').AsFloat;
                        ICMS.vICMSSTRet := qrnota_item.fieldbyname('icms_sub').AsFloat;
                      end;
                    csosn900:
                      begin
                        ICMS.modBC := dbiMargemValorAgregado;
                        ICMS.vBC := qrnota_item.fieldbyname('base_calculo').AsFloat;
                        ICMS.pRedBC := qrnota_item.fieldbyname('icms_reduzido').AsFloat;
                        ICMS.pICMS := qrnota_item.fieldbyname('ICMS').asfloat;
                        ICMS.vICMS := qrnota_item.fieldbyname('valor_icms').AsFloat;
                        ICMS.modBCST := dbisMargemValorAgregado;
                        ICMS.pMVAST := qrnota_item.fieldbyname('margem_agregada').AsFloat;
            ...
            if VersaoNFE = v400 then begin
              if qrnota_item.fieldbyname('pFCP').asfloat > 0 then
                ICMS.pFCP := qrnota_item.fieldbyname('pFCP').asfloat;
              if qrnota_item.fieldbyname('vBCFCP').asfloat > 0 then
                ICMS.vBCFCP := qrnota_item.fieldbyname('vBCFCP').asfloat;
              if qrnota_item.fieldbyname('vFCP').asfloat > 0 then
                ICMS.vFCP := qrnota_item.fieldbyname('vFCP').asfloat;
              if qrnota_item.fieldbyname('vBCFCPST').asfloat > 0 then
                ICMS.vBCFCPST := qrnota_item.fieldbyname('vBCFCPST').asfloat;
              if qrnota_item.fieldbyname('pFCPST').asfloat > 0 then
                ICMS.pFCPST := qrnota_item.fieldbyname('pFCPST').asfloat;
              if qrnota_item.fieldbyname('vFCPST').asfloat > 0 then
                ICMS.vFCPST := qrnota_item.fieldbyname('vFCPST').asfloat;
              if qrnota_item.fieldbyname('pFCPSTRet').asfloat > 0 then
                ICMS.pFCPSTRet := qrnota_item.fieldbyname('pFCPSTRet').asfloat;
              if qrnota_item.fieldbyname('vFCPSTRet').asfloat > 0 then
                ICMS.vFCPSTRet := qrnota_item.fieldbyname('vFCPSTRet').asfloat;
              if qrnota_item.fieldbyname('pST').asfloat > 0 then
                ICMS.pST := qrnota_item.fieldbyname('pST').asfloat;
            end;
            //  lei transparencia
            with qrTrib do begin
              if (qrnota_item.fieldbyname('origem').AsInteger = 0) or (qrnota_item.fieldbyname('origem').AsInteger = 3) or (qrnota_item.fieldbyname('origem').AsInteger = 4) or (qrnota_item.fieldbyname('origem').AsInteger = 5) or (qrnota_item.fieldbyname('origem').AsInteger = 8) then begin
                close;
                sql.clear;
                sql.add('select * from IBPT_ALIQUOTAS where NCM =''' + qrnota_item.fieldbyname('CLASSIFICACAO_FISCAL').asstring + '''');
                open;
                if qrTrib.RecordCount > 0 then begin
                  vTotTrib := RoundTo((qrnota_item.fieldbyname('total').AsFloat * qrTrib.fieldbyname('ALIQNAC').asfloat) / 100, -2);
                  xTotalImposto := xTotalImposto + vTotTrib;
                end;
                TotalIBPT := RoundTo(TotalIBPT + ((Prod.vProd * qrTrib.fieldbyname('ALIQNAC').asfloat) / 100), -2);
              end
                ...
                Open;
                if qrTrib.RecordCount > 0 then begin
                  vTotTrib := RoundTo((qrnota_item.fieldbyname('total').AsFloat * qrTrib.fieldbyname('ALIQIMP').asfloat) / 100, -2);
                  xTotalImposto := xTotalImposto + vTotTrib;
                end;
                TotalIBPT := RoundTo(TotalIBPT + ((Prod.vProd * qrTrib.fieldbyname('ALIQIMP').asfloat) / 100), -2);
              end;
            end;
            if (Ide.idDest = doInterestadual) and (Dest.indIEDest = inNaoContribuinte) and (qrNFE_Cliente.fieldbyname('uf').asstring <> 'EX') then begin// Venda InterEstadual a Consumidor
              // obter aliquotas UF Destino
              if (Dest.indIEDest = inNaoContribuinte) then begin
                qryaux.Close;
                qryaux.SQL.Clear;
                qryaux.SQL.Add('SELECT ALIQ, ALIQST, ALIQIMPORTADO, FCP FROM ICM_PARTILHA WHERE UF=''' + qrNFE_Cliente.fieldbyname('uf').asstring + '''');
                qryaux.Open;
                lALiqICmsInterEstadual := qryaux.FieldByName('ALIQ').asfloat;  // INTERESTADUAL     7 / 12
                ...
                lDifal := RoundTo((lALiqICmsInterno - lAliqAux), -2);
                lBaseProduto1 := RoundTo((Prod.qTrib * Prod.vUnTrib), -2);
                lBaseProduto := RoundTo((lBaseProduto1 + Prod.vFrete + Prod.vSeg + Prod.vOutro + IPI.vIPI) - Prod.vDesc, -2);

                lIcmDifal := RoundTo((lBaseProduto / 100) * lDifal, -2);
                lValFCP := RoundTo((lBaseProduto / 100) * lAliqFCP, -2);
                lPartDest := RoundTo((lIcmDifal / 100) * wPercentPartilha, -2);
                lPartOrig := RoundTo((lIcmDifal / 100) * (100 - wPercentPartilha), -2);
                ...
                wVTotFPC := RoundTo(wVTotFPC + lValFCP, -2);
                wVTotPartDest := RoundTo(wVTotPartDest + lPartDest, -2);
                wVTotPartOrig := RoundTo(wVTotPartOrig + lPartOrig, -2);

                if frmmodulo.qrFilial.FieldByName('CRT').AsString = '3' then
                  InfAdic.InfCpl := InfAdic.InfCpl + ';** EC 87/2015 DIFAL ' + 'valor GNRE UF destinatário ' + FormatFloat('#0.00', wPercentPartilha) + '% R$ ' + FormatFloat('####,##0.00', wVTotPartDest) + ', ' + 'valor GNRE UF remetente ' + FormatFloat('#0.00', 100 - wPercentPartilha) + '% R$ ' + FormatFloat('####,##0.00', wVTotPartOrig) + ', ' + 'valor FCP R$ ' + FormatFloat('####,##0.00', wVTotFPC) + ' **'
                else
                  InfAdic.InfCpl := InfAdic.InfCpl + ';** EC 87/2015 DIFAL ' + 'valor GNRE UF destinatário ' + FormatFloat('#0.00', wPercentPartilha) + '% R$ ' + FormatFloat('####,##0.00', wVTotPartDest) + ', ' + 'valor GNRE UF remetente ' + FormatFloat('#0.00', 100 - wPercentPartilha) + '% R$ ' + FormatFloat('####,##0.00', wVTotPartOrig) + ', ' + 'valor FCP R$ ' + FormatFloat('####,##0.00', wVTotFPC) + ' **';
    ...
    if VersaoNFE = v400 then begin
      if qrnota.fieldbyname('vFCP').asfloat > 0 then
        Total.ICMSTot.vFCP := qrnota.fieldbyname('vFCP').asfloat;
      if qrnota.fieldbyname('vFCPST').asfloat > 0 then
        Total.ICMSTot.vFCPST := qrnota.fieldbyname('vFCPST').asfloat;
      if qrnota.fieldbyname('vFCPSTRet').asfloat > 0 then
        Total.ICMSTot.vFCPSTRet := qrnota.fieldbyname('vFCPSTRet').asfloat;
    end;
    ...
    if qrnota.fieldbyname('ret_valor_cofins').asfloat > 0 then
      Total.retTrib.vRetCOFINS := qrnota.fieldbyname('ret_valor_cofins').asfloat;
    if qrnota.fieldbyname('ret_valor_csll').asfloat > 0 then
      Total.retTrib.vRetCSLL := qrnota.fieldbyname('ret_valor_csll').asfloat;
    if qrnota.fieldbyname('ret_valor_bc_irrf').asfloat > 0 then
      Total.retTrib.vBCIRRF := qrnota.fieldbyname('ret_valor_bc_irrf').asfloat;
    if qrnota.fieldbyname('ret_valor_irrf').asfloat > 0 then
      Total.retTrib.vIRRF := qrnota.fieldbyname('ret_valor_irrf').asfloat;
    if qrnota.fieldbyname('ret_valor_bc_prev_soc').asfloat > 0 then
      Total.retTrib.vBCRetPrev := qrnota.fieldbyname('ret_valor_bc_prev_soc').asfloat;
    if qrnota.fieldbyname('ret_valor_prev_soc').asfloat > 0 then
      Total.retTrib.vRetPrev := qrnota.fieldbyname('ret_valor_prev_soc').asfloat;
    if (Ide.idDest = doInterestadual) and (Dest.indIEDest = inNaoContribuinte) then begin
    ...
    if VersaoNFE = v310 then begin
      if qrnota.fieldbyname('fat_qtde_prestacao').asinteger > 0 then begin
        ...
        for i := 1 to qrnota.fieldbyname('fat_qtde_prestacao').asinteger do begin
          with cobr.Dup.add do begin
            case i of
              1:
                begin
                  qrduplicatas.Close;
                  qrduplicatas.Params[0].AsString := qrnota.fieldbyname('CODIGO').AsString;
                  qrduplicatas.Params[1].AsString := '01';
                  qrduplicatas.Open;
                  nDup := qrduplicatas.fieldbyname('numero').asstring;
                  dVenc := qrduplicatas.fieldbyname('vencimento').AsDateTime;
                  vDup := RoundTo(qrduplicatas.fieldbyname('valor').asfloat, -2);
                end;
              2:
                begin
                  qrduplicatas.Close;
                  qrduplicatas.Params[0].AsString := qrnota.fieldbyname('CODIGO').AsString;
                  qrduplicatas.Params[1].AsString := '02';
                  qrduplicatas.Open;
                  nDup := qrduplicatas.fieldbyname('documento').asstring;
                  dVenc := qrduplicatas.fieldbyname('vencimento').AsDateTime;
                  vDup := RoundTo(qrduplicatas.fieldbyname('valor').asfloat, -2);
                end;
              3:
                begin
                  qrduplicatas.Close;
                  qrduplicatas.Params[0].AsString := qrnota.fieldbyname('CODIGO').AsString;
                  qrduplicatas.Params[1].AsString := '03';
                  qrduplicatas.Open;
                  nDup := qrduplicatas.fieldbyname('documento').asstring;
                  dVenc := qrduplicatas.fieldbyname('vencimento').AsDateTime;
                  vDup := RoundTo(qrduplicatas.fieldbyname('valor').asfloat, -2);
              ...
              4:
                begin
                  qrduplicatas.Close;
                  qrduplicatas.Params[0].AsString := qrnota.fieldbyname('CODIGO').AsString;
                  qrduplicatas.Params[1].AsString := '04';
                  qrduplicatas.Open;
                  nDup := qrduplicatas.fieldbyname('documento').asstring;
                  dVenc := qrduplicatas.fieldbyname('vencimento').AsDateTime;
                  vDup := RoundTo(qrduplicatas.fieldbyname('valor').asfloat, -2);
              ...
              5:
                begin
                  qrduplicatas.Close;
                  qrduplicatas.Params[0].AsString := qrnota.fieldbyname('CODIGO').AsString;
                  qrduplicatas.Params[1].AsString := '05';
                  qrduplicatas.Open;
                  nDup := qrduplicatas.fieldbyname('documento').asstring;
                  dVenc := qrduplicatas.fieldbyname('vencimento').AsDateTime;
                  vDup := RoundTo(qrduplicatas.fieldbyname('valor').asfloat, -2);
              ...
              6:
                begin
                  qrduplicatas.Close;
                  qrduplicatas.Params[0].AsString := qrnota.fieldbyname('CODIGO').AsString;
                  qrduplicatas.Params[1].AsString := '06';
                  qrduplicatas.Open;
                  nDup := qrduplicatas.fieldbyname('documento').asstring;
                  dVenc := qrduplicatas.fieldbyname('vencimento').AsDateTime;
                  vDup := RoundTo(qrduplicatas.fieldbyname('valor').asfloat, -2);
      ...
      if ide.finNFe = fnDevolucao then begin
        with pag.Add do begin
          tPag := fpSemPagamento;
          vPag := 0;
        end;
      end else begin
        if qrnotaOPERACAO.AsString = 'V' then begin
          with pag.Add do begin
            indPag := ipVista;
            tPag := fpDinheiro;
          ...
          if qrnota.fieldbyname('fat_qtde_prestacao').asinteger > 0 then begin
            ...
            qrduplicatas.First;
            TotalPago := 0;
            while not qrduplicatas.Eof do begin
              with cobr.Dup.add do begin
                nDup := qrduplicatas.fieldbyname('NUMERO').asstring;
                dVenc := qrduplicatas.fieldbyname('vencimento').AsDateTime;
                vDup := RoundTo(qrduplicatas.fieldbyname('valor').asfloat, -2);
                if qrduplicatas.fieldbyname('vencimento').AsDateTime <= date then
                  TotalPago := TotalPago + RoundTo(qrduplicatas.fieldbyname('valor').asfloat, -2);
              end;
              qrduplicatas.Next;
            end;
            with pag.Add do begin
    ...
    if (Transp.modFrete <> mfContaEmitente) and (Transp.modFrete <> mfSemFrete) then begin
      ...
      Transp.Transporta.xEnder := Trim(qrtransportador2.fieldbyname('endereco').asstring) + ', ' + Trim(qrtransportador2.fieldbyname('numero').asstring);
      ...
      if qrnota.fieldbyname('frete_conta').asinteger = 9 then begin
    ...
    if xTotalImposto > 0 then
      InfAdic.InfCpl := InfAdic.InfCpl +  ';' + ' Valor aproximado tributos R$' + FormatFloat('#,##0.00', xTotalImposto) + ' (' + FormatFloat('#0.00', PorcIBPT) + '%) Fonte: IBPT';

    InfAdic.infCpl := InfAdic.infCpl + '; Forma de Pagamento: ' + qrnotaFAT_FORMA_PGTO.asstring;
...
begin
  ...
  if sUsa_NFE = '1' then begin
    qrnota.fieldbyname('MODELO_NF').AsString := '55';
    qrnota.fieldbyname('ESPECIE').AsString := 'NFE';
  end;
  ...
  qrnota.fieldbyname('SERIE_NF').AsString := qrempresaCONHECIMENTO.AsString;
...
begin
  ...
  if (qrnota.fieldbyname('nfe_situacao').asstring = '6') then begin
    Application.MessageBox('Não é possivel executar esta ação em uma NF-e Enviada', 'Atenção!', MB_ICONINFORMATION);
    Exit;
  end;
  if (qrnota.fieldbyname('nfe_situacao').asstring = '8') then begin
    Application.MessageBox('Não é possivel executar esta ação em uma NF-e Cancelada', 'Atenção!', MB_ICONINFORMATION);
    Exit;
  end;
  if (qrnota.fieldbyname('nfe_situacao').asstring = '5') then begin
    Application.MessageBox('Não é possivel executar esta ação em uma NF-e que está aguardando o retorno', 'Atenção!', MB_ICONINFORMATION);
    Exit;
  end;
...
begin
  ...
  if frmPrincipal.autentica('Excluir NF', 4) then begin
    query2.close;
    query2.sql.clear;
    ...
    if query2.RecordCount > 0 then begin
      Application.messagebox('Esta nota fiscal possue prestações que já foram feito pagamentos! Favor verificar!', 'Aviso', mb_ok + mb_iconerror);
      exit;
    end;
    query2.close;
    query2.sql.clear;
    ...
    if qrnota.fieldbyname('SITUACAO').asinteger = 1 then begin
      ...
      qrnota_item.open;
      while qrnota_item.RecordCount <> 0 do begin
        if qrnota_item.fieldbyname('CODLANCAMENTO').asstring <> 'XXXXXX' then begin
        ...
        if qrnota.fieldbyname('BAIXA_ESTOQUE').asstring = 'S' then begin
          if qrnota_item.fieldbyname('codgrade').asstring <> '' then begin
            if frmnotafiscal_menu.qrnota.fieldbyname('BAIXA_ESTOQUE').asstring = 'S' then begin
              ...
              if frmmodulo.qrgrade_produto.RecordCount > 0 then begin
                ...
                if frmnotafiscal_menu.qrnota.fieldbyname('MOVIMENTO').asstring = 'S' then
                  frmmodulo.qrgrade_produto.fieldbyname('estoque').asfloat := frmmodulo.qrgrade_produto.fieldbyname('estoque').asfloat + qrnota_item.fieldbyname('qtde').asfloat
                else
                  frmmodulo.qrgrade_produto.fieldbyname('estoque').asfloat := frmmodulo.qrgrade_produto.fieldbyname('estoque').asfloat - qrnota_item.fieldbyname('qtde').asfloat;
          ...
          if qrnota_item.fieldbyname('serial').asstring <> '' then begin
            query.close;
            query.sql.clear;
...
begin
  ...
  if (ENUMERO.Text = '000000') or (ENUMERO.Text = '') then begin
...
begin
  ...
  if qrnota.RecordCount = 0 then begin
    Application.MessageBox('Não existe nota fiscal para cancelar', 'Atenção!', MB_ICONINFORMATION);
    exit;
  end;

  if qrnota.fieldbyname('nfe_situacao').asinteger = 8 then begin
    Application.MessageBox('Nota fiscal já esta cancelada.', 'Atenção!', MB_ICONINFORMATION);
    exit;
  end;

  if frmPrincipal.autentica('Cancelar NF', 4) then begin
    ...
    if qrnota.fieldbyname('nfe_situacao').asinteger = 6 then begin

      sXML := qrnota.fieldbyname('nfe_xml').asstring;
      if sXML = '' then begin
        Application.MessageBox('Nota fiscal sem XML, impossivel efetuar o cancelamento.', 'Atenção!', MB_ICONINFORMATION);
        exit;
      end;
      ...
      with ACBrNFe1.EventoNFe.Evento.Add do begin
        //infEvento.chNFe      := qrnotaCHAVE.AsString;
      ...
      if ACBrNFe1.EnviarEvento(NumeroLote) then begin
        with ACBrNFe1.WebServices.EnvEvento do begin
          if EventoRetorno.retEvento.Items[0].RetInfEvento.cStat <> 135 then begin
            raise Exception.CreateFmt('Ocorreu o seguinte erro ao cancelar a nota fiscal eletrônica:' + sLineBreak + 'Código:%d' + sLineBreak + 'Motivo: %s', [EventoRetorno.retEvento.Items[0].RetInfEvento.cStat, EventoRetorno.retEvento.Items[0].RetInfEvento.xMotivo]);
          end;
          ...
          //Criar campos no banco para gravar as informações acima
    ...
    while not qrnota_item.eof do begin
      ...
      if Trim(qrnota_item.fieldbyname('serial').asstring) <> '' then begin
        query.close;
        query.sql.clear;
        query.sql.add('update c000022 set situacao = :situacao, cliente = :cliente, codcliente = :codcliente, datavenda = :datavenda,');
        query.sql.add('codvenda = :codvenda, precovenda = :precovenda');
    ...
    if self.Tag = 100 then begin
      ...
      try
        ...
        frmmodulo.QRVENDA.sql.add('SELECT * FROM C000048 ORDER BY DATA, codigo');
        frmmodulo.QRVENDA.open;
        frmmodulo.QRVENDA.IndexFieldNames := 'CODIGO';
        if frmmodulo.QRVENDA.Locate('NUMERO_CUPOM_FISCAL', qrnota.fieldbyname('NUMERO').asstring, [loCaseInsensitive]) then
...
procedure Tfrmnotafiscal_menu.gridTitleButtonClick(Sender: TObject; AFieldName: string);
var
  cliente, nota: string;
begin

  if (ansiuppercase(AFieldName) = ansiuppercase('numero')) or (ansiuppercase(AFieldName) = ansiuppercase('data')) or (ansiuppercase(AFieldName) = ansiuppercase('data_saida')) or (ansiuppercase(AFieldName) = ansiuppercase('modelo')) or (ansiuppercase(AFieldName) = ansiuppercase('serie')) or (ansiuppercase(AFieldName) = ansiuppercase('crop')) or (ansiuppercase(AFieldName) = ansiuppercase('total_nota')) or (ansiuppercase(AFieldName) = ansiuppercase('base_calculo')) or (ansiuppercase(AFieldName) = ansiuppercase('valor_icms')) then begin
    if ecliente.Text = '' then
      cliente := ''
    else
      cliente := ' and codcliente = ''' + Copy(ecliente.Text, 1, 6) + '''';
  ...
  if ansiuppercase(AFieldName) = 'CLIENTE' then begin
    if ecliente.Text = '' then
      cliente := ''
    else
      cliente := ' and codcliente = ''' + Copy(ecliente.Text, 1, 6) + '''';
...
begin
  ...
  if frmPrincipal.autentica('Zerar NF', 4) then begin
...
procedure Tfrmnotafiscal_menu.FormClose(Sender: TObject; var Action: TCloseAction);
begin

  Action := cafree;
end;

procedure Tfrmnotafiscal_menu.FormCreate(Sender: TObject);
begin
  ACBrNFe1.Configuracoes.WebServices.Salvar := true;

  qrnotaRET_VALOR_PIS.DisplayFormat := mascara_valor;
  qrnotaRET_VALOR_COFINS.DisplayFormat := mascara_valor;
  qrnotaRET_VALOR_CSLL.DisplayFormat := mascara_valor;
  qrnotaRET_VALOR_BC_IRRF.DisplayFormat := mascara_valor;
  qrnotaRET_VALOR_IRRF.DisplayFormat := mascara_valor;
  qrnotaRET_VALOR_BC_PREV_SOC.DisplayFormat := mascara_valor;
  qrnotaRET_VALOR_PREV_SOC.DisplayFormat := mascara_valor;
end;
...
begin
  if Application.messagebox('Confirma a exportação desta nota fiscal para o sistema de Emissão de Nota Fiscal Eletrônica?', 'Aviso', mb_yesno + mb_iconquestion) = idyes then begin
    // filial
    query2.close;
    query2.sql.clear;
    query2.sql.add('select * from c000004');
    ...
    if Exp_Abre_NOTAFISCAL(inputbox('NFe', 'Nome do Arquivo', 'NF' + qrnota.fieldbyname('numero').asstring + '.txt')) then begin
      ...
      if not Exp_Inicia_NotaFiscal(1, query2.fieldbyname('uf').asstring, qrnota.fieldbyname('natureza').asstring, Fat,
      ...
      while not qrnota_item.eof do begin
        ...
        if not Exp_Inclui_item(qrnota_item.fieldbyname('item').asinteger,
          ...
          qrnota_item.fieldbyname('cest').asstring, qrnota_item.fieldbyname('cfop').asstring, // CFOP,
      ...
      if Fat = 1 then begin
        ...
        if not Exp_Faturamento_NotaFiscal(frmPrincipal.zerarcodigo(qrnota.fieldbyname('numero').asstring, 9),
        ...
        while not query2.eof do begin
          if not Exp_Duplicata_NotaFiscal(query2.fieldbyname('documento').asstring, // Numero:string;
      ...
      if Exp_Fecha_NotaFiscal(qrnota.fieldbyname('inf1').asstring + ' ' + qrnota.fieldbyname('inf2').asstring + ' ' + qrnota.fieldbyname('inf3').asstring + ' ' + qrnota.fieldbyname('inf4').asstring + ' ' + qrnota.fieldbyname('inf5').asstring) then begin
...
procedure Tfrmnotafiscal_menu.PathClick(Sender: TObject);
var
  Dir: string;
begin
...
begin
  ...
  if sXML = '' then
    Exit;
  try
    if qrnota.FieldByName('nfe_situacao').AsInteger = 4 then begin
      if FileExists(sXML) then begin
        ...
        ACBrNFe1.Consultar;
        if acbrnfe1.WebServices.Consulta.cStat = 100 then begin
    ...
    on E: exception do begin
      Application.messagebox(PWideChar('004- Erro ao crair/imprimir NFe!' + #13 + 'Mensagem: ' + E.message), 'Erro', mb_ok + mb_iconerror);
    end;
...
//  bt_log.Click;
  Application.MessageBox(PWideChar('Status Serviço'+#13+
                         ...
                         'xMotivo: '  +ACBrNFe1.WebServices.StatusServico.xMotivo+#13+
                         'cUF: '      +IntToStr(ACBrNFe1.WebServices.StatusServico.cUF)+#13+
                         ...
                         'xObs: '     +ACBrNFe1.WebServices.StatusServico.xObs),'Atenção!',MB_ICONINFORMATION);
...
begin
  case qrnota.fieldbyname('nfe_situacao').asinteger of
    ...
    4:
      begin // validada
        if Application.messagebox('Esta nota já foi validada! Deseja prosseguir?', 'Atenção', mb_yesno + MB_ICONWARNING + MB_DEFBUTTON2) = idno then
          exit;
      end;
    5:
      begin // transmitida
        if Application.messagebox('Esta nota já foi validada e transmitida! Deseja prosseguir?', 'Atenção', mb_yesno + MB_ICONWARNING + MB_DEFBUTTON2) = idno then
          exit;
      end;

    6:
      begin // Aceita
        if Application.messagebox('Esta nota já foi validada/aceita pela receita! Deseja prosseguir?', 'Atenção', mb_yesno + MB_ICONWARNING + MB_DEFBUTTON2) = idno then
          exit;
      end;

    7:
      begin // rejeitada
        if Application.messagebox('Esta nota já foi validada e rejeitada! Deseja prosseguir?', 'Atenção', mb_yesno + MB_ICONWARNING + MB_DEFBUTTON2) = idno then
          exit;
      end;
    8:
      begin // cancelada
        Application.messagebox('Esta nota já foi cancelada!', 'Erro', mb_ok + mb_iconerror);
        exit;
      end;
  end;
  ...
  try
    ...
    ACBrNFe1.NotasFiscais.Imprimir;
    if Application.MessageBox('Confirma o envio desta NF-e?', 'Atenção!', MB_ICONINFORMATION + MB_YESNO) <> mrYes then
      Exit;
    ...
    if FileExists(sXML) then begin
      ...
      qrnota.fieldbyname('nfe_xml').asstring := sXML;
      qrnota.fieldbyname('nfe_situacao').asinteger := 4;
      qrnota.fieldbyname('chave').AsString := Copy(ACBrNFe1.NotasFiscais.Items[0].NFe.infNFe.ID, (Length(ACBrNFe1.NotasFiscais.Items[0].NFe.infNFe.ID) - 44) + 1, 44);
      qrnota.post;
...
begin
  ...
  if FileExists(sXML) then begin
    ACBrNFe1.NotasFiscais.clear;
    ACBrNFe1.NotasFiscais.LoadFromFile(sXML);
    ACBrNFe1.Enviar(StrToInt(FormatDateTime('yymmddhhmm', NOW)));
    ACBrNFe1.NotasFiscais.Imprimirpdf;
...
begin
  try
    ...
    if FileExists(sXML) then begin
      frmmodulo.qrconfig.open;
      if frmmodulo.qrconfigEMAIL_SERVIDOR_SMTP.AsString = '' then begin
        Application.messagebox('Favor configurar a conta de email!', 'Erro', mb_ok + mb_iconerror);
        exit;
      end;

      pdf := 'C:\TALOS\SERVER\PDF\' + stringreplace(ExtractFileName(sXML), '-NFe.xml', '.pdf', [rfReplaceAll, rfIgnoreCase]);
      ...
      try
        //CC.Add('hacmoni.07@gmail.com'); //especifique um email válido
        //CC.Add('hacmoni.07@gmail.com');    //especifique um email válido
        ACBrMail1.Clear;
        ...
        ACBrMail1.From := frmmodulo.qrDFeConfigNFE_EMAIL_REMETENTE.AsString;
      ...
      Application.messagebox('XML não encontrado na pasta de xmls!', 'Atenção!', MB_ICONINFORMATION);
...
var
  ...
  dhRecbto: TDateTime;
begin
  Application.CreateForm(TfrmJustificativaInutilizacaoNFe, frmJustificativaInutilizacaoNFe);
  frmJustificativaInutilizacaoNFe.ShowModal;
  if not frmJustificativaInutilizacaoNFe.Continua then
    Exit;
  Modelo := frmJustificativaInutilizacaoNFe.edModelo.Text;
  Serie := frmJustificativaInutilizacaoNFe.edSerie.Text;
  Ano := frmJustificativaInutilizacaoNFe.edAno.Text;
  NumeroInicial := frmJustificativaInutilizacaoNFe.edNumero.Text;
  NumeroFinal := frmJustificativaInutilizacaoNFe.edNumero.Text;
  Justificativa := frmJustificativaInutilizacaoNFe.edJustificativa.Lines.Text;
  FreeAndNil(frmJustificativaInutilizacaoNFe);
  qryTmp.Close;
  qryTmp.SQL.Text := 'select * from c000061 where numero = ' + QuotedStr(NumeroInicial);
  qryTmp.Open;
  if not qryTmp.IsEmpty then begin
    if (qryTmp.FieldByName('nfe_situacao').AsString = '8') and not (qryTmp.FieldByName('chave').IsNull) then begin
      Application.MessageBox('Não é necessário Inutilizar uma numeração de uma NF ja Cancelada.', 'Atenção!', MB_ICONINFORMATION);
      Exit;
    end;
    if (qryTmp.FieldByName('nfe_situacao').AsString = '6') then begin
      Application.MessageBox('Não é posivel inutilizar um NF enviada.', 'Atenção!', MB_ICONINFORMATION);
      Exit;
    end;
  end;

  ACBrNFe1.WebServices.Inutiliza(frmmodulo.qrFilialCNPJ.AsString, Justificativa, StrToInt(Ano), StrToInt(Modelo), StrToInt(Serie), StrToInt(NumeroInicial), StrToInt(NumeroFinal));
  Application.CreateForm(TfrmResultadoWebService, frmResultadoWebService);
  frmResultadoWebService.pnTexto.Caption := IntToStr(ACBrNFe1.WebServices.Consulta.cStat) + ' - ' + ACBrNFe1.WebServices.Consulta.xMotivo;
  frmResultadoWebService.LoadXML(ACBrNFe1.WebServices.Inutilizacao.RetornoWS, frmResultadoWebService.WBResposta);
  frmResultadoWebService.ShowModal;
  FreeAndNil(frmResultadoWebService);

  //Gravar no Banco
  tpAmb := TpAmbToStr(ACBrNFe1.WebServices.Inutilizacao.TpAmb);
  dhRecbto := ACBrNFe1.WebServices.Inutilizacao.dhRecbto;
  Protocolo := ACBrNFe1.WebServices.Inutilizacao.Protocolo;
...
begin
  ...
  if not RetornaTexto('Carta de Correção - Correção a ser considerada', Correcao, True, 539, 661, True, 15, 300) then
    Exit;
  ACBrNFe1.EventoNFe.Evento.Clear;
  with ACBrNFe1.EventoNFe.Evento.Add do begin
  ...
  if ACBrNFe1.EnviarEvento(StrToInt(idLote)) then begin
    with ACBrNFe1.WebServices.EnvEvento do begin
      if not (EventoRetorno.retEvento.Items[0].RetInfEvento.cStat in [135, 136]) then begin
        raise EDatabaseError.CreateFmt('Ocorreu o seguinte erro ao enviar a carta de correção:' + sLineBreak + 'Código:%d' + sLineBreak + 'Motivo: %s', [EventoRetorno.retEvento.Items[0].RetInfEvento.cStat, EventoRetorno.retEvento.Items[0].RetInfEvento.xMotivo]);
      end;
       // retornos
      DataHoraEvento := EventoRetorno.retEvento.Items[0].RetInfEvento.dhRegEvento;
      NumeroProtocolo := EventoRetorno.retEvento.Items[0].RetInfEvento.nProt;
