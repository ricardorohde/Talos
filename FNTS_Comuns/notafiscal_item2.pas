...
uses
  ...
  cxLookAndFeels, cxLookAndFeelPainters, dxBarBuiltInMenu, cxPC, dxSkinsCore,
...
begin
  ...
  begin
    ...
    FrmNotaFiscal.qrnotafiscal_item.sql.add('pFCP,vBCFCP,vFCP,vBCFCPST,pFCPST,vFCPST,pFCPSTRet,vFCPSTRet,pST, xped, xitem)');
    ...
    if ed_produto.text <> '' then begin
      if loc_produto(ed_produto.text) then begin
        if ed_produto.Text = '' then begin
          ...
          if blancar_total.Checked = true then begin
  ...
  if ed_cfop.text <> '' then begin
    ...
    if query1.recordcount > 0 then begin
      ...
      if frmmodulo.qrFilial.FieldByName('OPTANTE_SIMPLES').AsString = 'S' then begin
        ...
        if ed_icms_aliquota.Value <> 0 then begin
          if ed_icms_reducao.value <> 0 then begin
            if (ed_cst.text = '060') or (ed_cst.text = '010') or (ed_cst.text = '070') then begin
              margem_agregada := query1.fieldbyname('margem_agregada').Asfloat;
              if margem_agregada <> 0 then begin
            ...
            if ed_pis_aliquota.Value > 0 then begin
              ed_pis_base.value := ed_total.value;
              ed_pis_valor.value := ed_pis_base.value * (ed_pis_aliquota.value / 100);
            end;
            if ed_cofins_aliquota.Value > 0 then begin
          ...
          // aliquota de icms = 0
          if ed_cfop.Text = '6114' then begin // opção para a ouro preto  nota de consignação
            ed_icms_base.value := ed_total.value;
            ed_icms_valor.value := 0;
      ...
      if ed_ipi_aliquota.value <> 0 then begin
