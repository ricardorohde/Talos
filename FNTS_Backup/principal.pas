...
begin
  ...
  if not OpenDialog2.Execute then //se não selecionar o arquivo de backup
...
begin
  ...
  try
    ...
    Reg.WriteString('store_protheus_backup', Application.ExeName);
...
begin
   {Esconde o botão da barra de tarefas}
...
begin
  ...
  begin
    ...
    if cbtipo.Text = '' then
    begin
      showmessage('Agendamento é obrigatório');
      cbtipo.SetFocus;
      abort;
    end;
    ...
    begin
      showmessage('IP é obrigatório');
      dbedit1.SetFocus;
      abort;
    end;
    if DBEdit2.Text = '' then
    begin
      showmessage('Base é obrigatório');
      DBEdit2.SetFocus;
      abort;
    end;
    if DBEdit3.Text = '' then
    begin
      showmessage('Caminho para o backup gerado é obrigatório');
      DBEdit3.SetFocus;
      abort;
    end;
...
begin
  try
    with IBRestoreService1 do begin
      ...
      listbox1.Items.Add(Format('Restore Iniciado às %s horas', [FormatDateTime('hh:nn:ss', now)]));
      Application.ProcessMessages;
      listbox1.Items.Add('');
      try
      ...
      listbox1.Items.Add('');
      listbox1.items.add(Format('Restore Finalizado às %s horas', [FormatDateTime('hh:nn:ss', now)]));
      Screen.Cursor := CrDefault;
      base.Connected := True;
      tbagenda.Open;
      Timer1.Enabled := True;
    ...
    on E: Exception do begin
      listbox1.Items.Add('Ocorreu um erro inesperado. O restore não foi concluído.');
      listbox1.Items.Add('Informações da exceção:');
