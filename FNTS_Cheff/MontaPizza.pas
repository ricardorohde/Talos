...
begin
  ...
  with frmMontaPizza do Begin
    ...
    while not qrProdutos.eof do begin
      edSabor.Items.Add(qrProdutosPRODUTO.AsString);
      qrProdutos.Next;
    end;
...
begin
  if qrProdutos.Locate('produto',edSabor.Text,[loCaseInsensitive]) then begin
    strExtra := RetornaAdcional(qrProdutosCODIGO.AsString);
  end;
end;
...
begin
  ...
  try
    ...
    if qrProdutos.Locate('produto',edSabor.Text,[loCaseInsensitive]) then begin
      ...
      cdsSaboresDescricao.AsString := qrProdutosPRODUTO.AsString;
      cdsSaboresvalor.AsFloat := qrProdutosPRECOVENDA.AsFloat + strExtra.valor;
