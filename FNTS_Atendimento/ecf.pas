...
begin
  ...
  IF FileExists('C:\TALOS\RETORNO.TXT') THEN DeleteFile('C:\TALOS\RETORNO.TXT');
  ...
  begin
    ...
    assignfile(f,'C:\TALOS\retorno.txt');
    rewrite(f);
...
begin
 ...
 texto := '';
  ...
  begin
       ...
       begin
         ...
         begin
      application.messagebox(pwidechar('Não foi possível executar o comando!'+#13+'Motivo: '+'Impressora Fiscal não responde.'),
        'Atenção', mb_ok + MB_ICONWARNING);
           RESULT := 'ERRO';
         end
         else
         begin
      application.messagebox(pwidechar('Não foi possível executar o comando!'+#13+'Motivo: '+copy(rt,11,100)+'.'),
        'Atenção', mb_ok + MB_ICONWARNING);
           RESULT := 'ERRO';
         end;
       end
 ...
 begin
  ...
  220 : texto  := 'Overflow COO';
   ...
   ELSE
   BEGIN
      application.messagebox(pwidechar('Não foi possível executar o comando!'+#13+'Motivo: '+texto+'.'),
        'Atenção', mb_ok + MB_ICONWARNING);
    RESULT := 'ERRO';
   END;
