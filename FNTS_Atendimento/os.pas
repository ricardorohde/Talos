...
uses
  ...
  wwdbedit, Wwdotdot, Wwdbcomb, Grids, Wwdbigrd, Wwdbgrid, DB, Menus, ZAbstractRODataset, ZDataset, ZAbstractDataset, Wwdbdlg,
  AdvShapeButton, AdvGlowButton, RzEdit, RzDBEdit, RzDBBnEd, maskutils, IBCustomDataSet, IBQuery, JvMemoryDataset, JvBaseEdits, JvDBControls, JvExMask, JvToolEdit;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
          if application.messagebox(pwidechar('Cliente com Prestação vencida a ' + inttostr(dias)+' Dias!, Deseja continuar?'),
            'Atenção', mb_yesno + MB_ICONERROR) = idno then
        begin
  ...
  2 : begin // em observacao
          if application.messagebox(pwidechar('Cliente com Crédito em Observação:'+#13+
  ...
  3 : begin // bloqueado
          ...
          if application.messagebox(pwidechar('Cliente com crédito Bloqueado!, Deseja continuar?'),
            'Atenção', mb_yesno + MB_ICONERROR) = idno then
          begin
  ...
  4 : begin // inativo
          if application.messagebox(pwidechar('Cliente INATIVO!, Deseja continuar?'),
            'Atenção', mb_yesno + MB_ICONERROR) = idno then
          begin
            if not frmPrincipal.autentica('Liberar venda',3) then
            begin
  ...
  limite_disponivel := qrcliente.FIELDBYNAME('LIMITE').ASFLOAT - vlimite;
     ...
     begin
       ...
       BEGIN
          if application.messagebox(pwidechar('Este cliente possue um limite disponível de:'+#13+' ---> R$ '+formatfloat('###,###,##0.00',(qrcliente.FIELDBYNAME('LIMITE').ASFLOAT - vlimite))+'  <--- '+#13+'É necessário autorização para prosseguir! '+#13+'Deseja continuar?'),
            'Atenção', mb_yesno + MB_ICONERROR) = idno then
         begin
           if not frmPrincipal.autentica('Liberar venda',3) then
           begin
